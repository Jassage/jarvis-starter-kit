...
### ANTENN : audit senior dev + designer, puis correction des 6 manques identifiés (audience/preuves de diffusion, RBAC, audit, comptes, moniteur, accessibilité) et premiers tests automatisés du portefeuille

**Contexte :** sur « agis comme un senior dev et comme un designer pour analyser ANTENN et dis-moi ce qui manque », lecture intégrale du backend et du frontend (le projet avait grossi depuis sa fiche : replay/VOD, EPG multi-jours, config chaîne, app mobile Expo). Puis « vas y » sans précision de portée : décision assumée d'enchaîner les 6 points de l'audit dans l'ordre recommandé plutôt que de bloquer sur une question, en documentant ce qui reste dehors.

**Verdict de l'audit :** le code métier est le mieux commenté du portefeuille et ses invariants sont réellement gardés côté service (chevauchement en bornes half-open dans une transaction, historique de grille figé, brouillon jamais exposé à l'EPG public, fenêtre de droits des replays, singleton de config verrouillé en base). **Mais le produit ne savait pas justifier sa raison d'être commerciale**, et les droits ne protégeaient pas ce qui engage la chaîne vis-à-vis d'un sponsor.

**1. Le livrable commercial était structurellement vide.** `DiffusionLog` n'était jamais écrit : une seule occurrence dans tout le backend, un `findMany` en lecture dans le rapport. Le rapport sponsors — seul document qui justifie une facturation sur un modèle FAST — retournait donc zéro diffusion et zéro vue pour toujours, et aucune source d'audience n'existait (ni le player web ni le mobile n'émettaient quoi que ce soit ; `nombreVuesEstimees` n'avait aucune origine). Chaîne complète construite : modèle `AudienceSession` (une ligne par couple session/créneau, clé opaque tirée par le player, aucune donnée personnelle) alimenté par un heartbeat de 30 s des deux players, **durée recalculée côté serveur et plafonnée à 60 s par ping** (un onglet mis en veille une heure n'ajoute jamais une heure de visionnage), ping refusé sur un créneau brouillon ou hors de sa plage de diffusion, génération **paresseuse et idempotente** des `DiffusionLog` (contrainte d'unicité base sur `creneauId`, déclenchée à l'ouverture du moniteur et des rapports faute de tâche de fond dans le projet), purge des sessions brutes à 90 jours. Le compteur de vues replay, jusque-là gonflable par une boucle curl sur une route publique sans déduplication, passe par le même mécanisme (une vue par session) et l'ancienne route `POST /replay/:id/vue` a été retirée des deux clients. **Limiteur de débit dédié et large sur le ping, explicitement exclu du limiteur global** : en Haïti une IP publique couvre couramment un quartier entier ou un cybercafé, plafonner à 300 requêtes/15 min aurait fait disparaître l'audience derrière une sortie NAT partagée.

**2. RBAC quasi absent au-delà des sponsors.** `requireAdministrateur` n'était câblé que sur les contrats : un OPERATEUR_REGIE pouvait modifier l'identité de la chaîne et son logo, supprimer un contenu, supprimer un replay et surtout **créer et supprimer les incrustations et bandeaux sponsors**, c'est-à-dire l'exécution matérielle du contrat commercial. Incohérence corrigée avec une ligne de partage explicite, documentée dans le README sous forme de tableau : **l'opérateur exploite l'antenne au quotidien, l'administrateur contracte et détruit**. Poser un habillage reste ouvert aux deux rôles, le retirer devient administrateur (cela prive un sponsor d'une exposition due), idem pour la suppression d'un contenu et la suppression définitive d'un replay (qui efface aussi les vues comptées dans le rapport). Sidebar filtrée par rôle : l'opérateur ne voit plus les entrées qui ne mèneraient qu'à un 403.

**3. Aucun journal d'audit** alors que le produit facture une exposition contractuelle : impossible de répondre à « qui a retiré ce bandeau et quand ». Modèle `AuditLog` ajouté (auteur figé en clair pour que la désactivation d'un compte ne vide pas l'historique, aucune route d'écriture ni de suppression exposée), 20 actions tracées (connexion, grille, contrats, habillage, identité d'antenne, replays, comptes), page `/journal` réservée à l'administrateur avec filtre par action.

**4. Gestion des utilisateurs livrée.** Elle était annoncée dans le README **et** dans un commentaire du middleware RBAC alors qu'aucun module n'existait : seul le seed créait des comptes, le client ne pouvait ni ajouter un opérateur ni désactiver un employé qui part, et un opérateur perdant son mot de passe restait bloqué jusqu'à une intervention directe en base. Module complet réservé à l'administrateur : création, changement de rôle, activation/désactivation révoquant immédiatement les sessions, anti auto-verrouillage (on ne peut ni changer son propre rôle ni se désactiver), garde du dernier administrateur actif, et **pas de suppression de compte** — volontairement, cela romprait le lien entre le journal d'audit et son auteur. Faute de SMTP sur ce déploiement, la réinitialisation passe par un **lien à usage unique** (jeton opaque haché SHA-256, 1 h) que l'administrateur transmet par le canal de son choix : l'intéressé choisit son mot de passe, l'administrateur ne le connaît jamais (même pattern que l'invitation client de POSTA). Politique de mot de passe unifiée et relevée à 10 caractères avec majuscule/minuscule/chiffre, contre un simple `min(8)` sans contrainte auparavant.

**5. L'écran principal manquait.** L'outil s'ouvrait sur `/grille`, un formulaire de programmation, alors qu'une régie a besoin d'un moniteur. Nouvelle page d'accueil `/moniteur` : ce qui est à l'antenne avec décompte du temps restant, programmes suivants, audience en direct ventilée web/mobile, et les quatre choses qui peuvent mal tourner (trou de grille dans les 24 h, créneaux jamais répercutés vers le playout, contrat sponsor expirant, absence de contenu de repli qui transformerait le moindre trou en écran noir). Aucune règle métier ajoutée : le module agrège des services qui existaient déjà et étaient répartis sur quatre pages.

**6. Volet design.** `:focus-visible` global ajouté (aucun style de focus n'existait hors des champs : sur fond navy, un opérateur au clavier ne voyait jamais où il était), bloc `prefers-reduced-motion` (le badge EN DIRECT clignotait en boucle infinie devant quelqu'un qui regarde l'écran huit heures d'affilée — cas d'école de WCAG 2.2.2), contraste du texte tertiaire relevé (≈3,4:1 sur `--color-surface`, sous le seuil AA de 4,5:1, et utilisé justement sur les plus petits textes de l'interface), système de toasts avec `aria-live` remplaçant des bandeaux d'erreur invisibles quand on agit en bas d'un tableau long et des succès qui ne disaient rien, dialogue de confirmation maison à la place de `window.confirm()` qui cassait le thème sombre en pleine action destructive, et modale enfin accessible (`role="dialog"`, `aria-modal`, piège à focus, fermeture par Échap, restitution du focus au déclencheur).

**Premiers tests automatisés du portefeuille (21, tous verts).** `node:test` intégré à Node 22, **aucune dépendance ajoutée**, sur les deux mécanismes dont une régression silencieuse coûterait le plus cher : la détection des trous de grille (algorithme de balayage extrait en fonction pure `calculerTrous`, testé sans base — adjacence, imbrication, débordement de fenêtre, trou d'une minute) et la garde de non-chevauchement (règle portée par une requête SQL, donc testée sur base réelle dans une plage horaire de l'année 2030, avec nettoyage automatique).

**Deux défauts du seed trouvés en vérifiant.** Il fabriquait un `DiffusionLog` à 480 vues sans aucune session derrière — exactement l'illusion à éviter en démonstration client, un rapport crédible alors que la chaîne d'audience ne fonctionne pas ; remplacé par de vraies sessions dont le log se déduit. Et son créneau « en cours » ne durait que 30 minutes : une demi-heure après le seed, la démonstration passait « hors antenne » avec un moniteur en alerte rouge. Allongé à 3 h.

**Incident d'outillage résolu, utile pour la suite :** `prisma migrate dev` refuse toujours de s'exécuter en non-interactif (limitation déjà documentée sur REMED), mais **`prisma migrate deploy` fonctionne parfaitement** — écrire la migration SQL à la main puis l'appliquer avec `deploy` est plus propre que le contournement par script Node utilisé jusqu'ici, et `migrate reset --force` fonctionne aussi (migrations + seed rejoués de zéro, testé trois fois).

**Vérifié en conditions réelles :** `tsc --noEmit` propre sur les trois bases de code (backend, frontend, mobile), 21/21 tests, **16/16 vérifications API sur serveur réellement démarré** (cycle d'audience complet, refus des pings hors antenne, idempotence de la génération des preuves, RBAC 403 par rôle sur config/comptes/journal, rapport sponsor passant de zéro à une exposition chiffrée), et **27/27 vérifications en navigateur réel** avec Playwright installé à la volée (heartbeat réellement émis par le player public et audience remontée jusqu'au moniteur de la régie, journal traçant les actions avec leur auteur, lien de réinitialisation généré puis consommé avec refus d'un mot de passe faible, sidebar filtrée par rôle, dialogue fermé par Échap, zéro erreur console). Trois défauts d'affichage repérés en lisant les captures d'écran et corrigés : liste « à suivre » illisible sans indication du jour (elle affichait 19:00, 20:00, 18:00 pour des créneaux de deux jours différents), durées de trou en minutes brutes (« 1266 min » au lieu de « 21 h 06 »), cartes de statistiques déséquilibrées. Le rate limiter d'authentification a d'ailleurs bloqué mes propres tests répétés (429), ce qui confirme au passage qu'il fonctionne. Base remise à l'état seed propre et serveurs arrêtés par PID en fin de session.

**Reste hors périmètre, documenté dans le README** : intégration ErsatzTV/RTMP/CDN réelle (le vrai chantier avant production), **alerte sortante sur trou d'antenne** (le moniteur ne prévient que si quelqu'un l'ouvre ; sans tâche de fond, un trou à 2 h du matin n'alerte personne), Docker et CI, couverture de test au-delà des deux mécanismes critiques. Jamais commité en git.

## 2026-08-13

### NEXORA : revue senior dev du code réel, puis durcissement sécurité (rate limiting, journal d'audit, test de garde RLS)

**Contexte :** sur « agis comme un senior dev, analyse NEXORA et dis-moi quoi faire », analyse du code réel plutôt que de la documentation ou de la mémoire de session.

**Ce qui s'est révélé solide (vérifié, pas supposé) :** isolation RLS complète, 12 tables métier et 12 policies `tenant_isolation`, toutes en `ENABLE` + `FORCE`, aucun oubli. Surface BYPASSRLS minuscule et justifiée (`AuthPrismaService` expose exactement 2 méthodes, login par email et invitation par token, les deux étant de vrais problèmes de bootstrap où le tenant n'est pas encore connu). Décrément de stock en compare-and-swap correct, avec fusion préalable des lignes dupliquées.

**Un soupçon infirmé par la mesure.** Le `DUMMY_HASH` anti-timing du login paraissait mal formé (61 caractères au lieu des 60 d'un hash bcrypt), ce qui aurait rendu la protection anti-énumération inopérante. Testé empiriquement avec le vrai `bcrypt` du projet : 741 ms contre 634 ms pour un hash réel, la protection mord effectivement. Rapporté comme tel à Jaslin plutôt que maquillé en trouvaille.

**Trois vrais trous trouvés, plus un correctif de config.**

1. **Aucun rate limiting nulle part.** `login`, `register`, `refresh` et l'acceptation d'invitation étaient ouverts au bruteforce, alors que POSTA, KONEKTE et BANKA en ont tous. `@nestjs/throttler` v6 ajouté, politiques centralisées dans `common/throttle.ts` plutôt que dispersées en décorateurs : garde-fou global 300/min via `APP_GUARD`, login 10/15min, register 5/h, refresh 60/15min (volontairement plus large, le token d'accès expirant en 15 min et plusieurs utilisateurs partageant souvent une IP derrière un NAT en Haïti), invitation 10/h. Ajout d'un `trust proxy` conditionnel (`TRUST_PROXY`, désactivé par défaut car l'activer sans proxy devant permettrait d'usurper son IP en forgeant l'en-tête) : sans lui, derrière nginx ou Railway, toutes les requêtes sembleraient venir du proxy et la limite de 10 deviendrait globale, coupant tous les clients d'un coup.

2. **Le journal d'audit n'existait que sur le papier.** La table `audit_logs` était créée, avec son RLS et sa policy, et pas une seule ligne n'y était jamais écrite (zéro occurrence dans `src/`). Branchée sur 9 actions à liste fermée (connexion réussie/échouée, déconnexion, changement de rôle, activation, désactivation, invitation, révocation, acceptation). Deux modes délibérément asymétriques et documentés : `recordInTx()` écrit DANS la transaction de l'action auditée (intégrité prioritaire, impossible de modifier un rôle sans laisser de trace) et `record()` non bloquant réservé au login (disponibilité prioritaire, un incident sur le journal ne doit pas empêcher un utilisateur légitime de se connecter). Un échec de connexion sur un compte **existant** est tracé (signal de ciblage), un email inconnu ne l'est pas et ne peut pas l'être, la table exigeant un `organizationId` et un `userId`.

3. **Le RLS reposait entièrement sur la discipline humaine.** Le test d'isolation existant prouvait que le RLS marche sur les tables qu'il connaît, mais rien ne garantissait qu'une table ajoutée plus tard soit protégée, et l'oubli aurait cassé en silence. Nouveau `test/rls-coverage.e2e-spec.ts` **sans aucune liste de tables en dur** : il interroge le catalogue PostgreSQL (`pg_class`, `pg_attribute`, `pg_policy`) pour découvrir les tables portant un `organization_id`, ajoute explicitement `organizations` (table racine du tenant, qui porte `id` et non `organization_id`), et exige de chacune ENABLE + FORCE + au moins une policy. Inclut un garde-fou sur le test lui-même (la requête de découverte doit renvoyer des lignes, sinon tous les contrôles passeraient à vide).

4. **`.env.example` mentait** en annonçant le port frontend 3010 au lieu de 3011, et le fallback CORS de `main.ts` avait la même valeur périmée : un clone frais suivant la doc démarrait avec un CORS cassé.

**Deux affirmations prouvées expérimentalement plutôt que déclarées.** L'atomicité de l'audit : une contrainte `CHECK ... NOT VALID` a été posée sur `audit_logs` pour saboter délibérément l'écriture d'audit, un vrai changement de rôle a été tenté via l'API, résultat HTTP 500 et **rôle resté inchangé** en base, confirmant que le rollback emporte bien les deux écritures ; contrainte retirée, même requête rejouée en 200. Et le fait que le test de garde RLS **mord vraiment** : une table `table_piege` avec un `organization_id` et sans protection a été créée, le test a échoué immédiatement en **nommant la table fautive** dans son message, puis retour à 10 tests verts après suppression.

**Vérifié en conditions réelles** : `tsc --noEmit` propre, 10 tests e2e verts, et surtout **démarrage réel du serveur** (leçon retenue de REMED, `tsc` seul ne suffit pas). 429 obtenu à la 11e tentative de login, pendant que `/auth/me`, `/dashboard/summary` et `/users` répondaient toujours 200, confirmant que le throttle est cloisonné par route et ne se transforme pas en auto-déni de service. Trace d'audit complète obtenue sur un cycle invitation → acceptation → changement de rôle → désactivation → réactivation, avec les bons auteurs et les bonnes cibles. Confirmation que les requêtes bloquées en 429 n'écrivent rien dans le journal (8 `LOGIN_FAILED` + 1 `LOGIN_SUCCESS` exactement, donc le bruteforce ne peut pas noyer la table d'audit). Base remise à son état initial (1 utilisateur, 1 organisation, 0 audit, 0 invitation), serveur arrêté par PID, port 4009 libre, aucun zombie.

**Incident à signaler : la clé API Anthropic réelle présente dans `backend/.env` s'est affichée en clair dans le transcript de la session**, une commande de vérification ayant raté sa censure (le motif de redaction ne correspondait pas au format `sk-ant-api03-`). Le fichier est correctement gitignoré et la clé n'a **jamais** été commitée, vérifié par `git log -S` sur tout l'historique. Rotation de la clé recommandée à Jaslin par précaution.

**Commité sur une branche dédiée** `durcissement/rate-limit-audit-rls` (`bc6216e`, 15 fichiers, 378 insertions) plutôt que directement sur `master` comme les trois commits de code précédents. Branche **non fusionnée** et **non poussée** (dépôt local sans remote, NEXORA n'étant pas déclaré comme sous-module du workspace parent). Commande de fusion fournie à Jaslin s'il préfère `master`.

**Points restants documentés, non traités** : 3 vulnérabilités `npm audit` de niveau high préexistantes et sans lien avec ces changements ; un changement de rôle qui met jusqu'à 15 min à prendre effet, le rôle étant lu depuis le JWT (compromis assumé du JWT sans état, la désactivation restant elle immédiate côté sessions grâce à la révocation des refresh tokens) ; le TOCTOU du quota Copilot et le coût du `forTenant()` (une transaction, donc deux allers-retours, par opération), tous deux jugés acceptables en MVP.

**Avis stratégique donné, inchangé depuis le 2026-07-26 et devenu plus pressant.** Le code n'est plus le goulot d'étranglement de NEXORA. La Phase 1 de sa propre roadmap (`NEXORA-OS/01-STRATEGY/company-roadmap.md`) est une phase de **validation terrain sans une seule ligne de code à écrire** : 15 à 20 entretiens structurés, choix tranché d'un vertical unique parmi hôtellerie/restauration/commerce, pilote gratuit. Le projet a toujours **zéro client**, dans un portefeuille de 19 applications. Contre-argument présenté honnêtement pour que Jaslin tranche en connaissance de cause : si l'objectif de NEXORA est une vitrine technique (portfolio, investisseur, recrutement) plutôt qu'un produit vendu, continuer à construire se défend, mais cela ne se pilote pas de la même façon et mérite d'être dit explicitement.

### NEXORA : verdict « prêt à encaisser des abonnements ? », puis finition UX et première passe navigateur

**Contexte :** deux questions de Jaslin en enchaînement, « est-ce que c'est complet à 100 % ? » puis « est-ce que c'est prêt à être mis en ligne pour que les utilisateurs paient leur abonnement ? ».

**La première question a fait tomber deux trous dans mon propre travail.** J'avais annoncé la session précédente comme « vérifiée en conditions réelles », ce qui était vrai mais incomplet : deux chemins de code que j'avais écrits n'avaient **jamais été exécutés une seule fois**, `AUTH_LOGOUT` (jamais appelé `/api/auth/logout` pendant les tests) et `INVITATION_REVOKED` (invitation et acceptation testées, révocation non). Exécutés depuis : les deux fonctionnent, y compris la garde anti-doublon du logout rejoué (second appel avec un cookie déjà révoqué, 200 sans seconde entrée en base). Également corrigée dans la foulée : une fausse alerte de ma part sur un échec de build frontend, en réalité un exit code 0 (compilation réussie, 13 pages générées), la trace prise pour une erreur étant un incident transitoire de chargement des polices Google.

**Réponse à la seconde question : non, et pas « presque ».** Vérifié par recherche exhaustive sur tout le backend et le frontend (`subscription`, `abonnement`, `stripe`, `moncash`, `billing`, `payment`, `plan`, `trial`, `checkout`) : **aucune ligne de code capable d'encaisser de l'argent**, l'unique résultat étant un commentaire disant que la facturation est absente du MVP. Les 12 modèles de la base ne comportent ni `Plan`, ni `Subscription`, ni `Payment`, ni `Invoice`. Aujourd'hui `register` crée une organisation avec un accès complet, illimité et gratuit à vie. **Deux autres trous rédhibitoires trouvés au passage** : aucun parcours de récupération de mot de passe (les routes d'auth se limitent à register/login/refresh/logout/me, un client enfermé dehors n'aurait aucun recours), et aucun artefact de déploiement (pas de Dockerfile applicatif, pas de CI, le seul `docker-compose.yml` servant au Postgres de développement local). Points de production également signalés : conditions générales et politique de confidentialité inexistantes (`12-LEGAL/` ne contient que des plans de rédaction), SMTP dont la délivrabilité réelle n'est pas prouvée, et compteurs de rate limiting en mémoire donc remis à zéro à chaque redémarrage et non partagés entre instances (un stockage Redis serait nécessaire en multi-instance, même besoin que l'adapter Socket.io de KONEKTE).

**Avis donné : ne pas construire la facturation maintenant**, non parce que c'est difficile mais parce que Jaslin ne sait pas encore quoi facturer, à qui, ni à quel prix, faute d'un seul entretien client. Recommandation concrète : montrer le produit gratuitement, et si un premier client veut payer, **l'encaisser à la main** (MonCash direct, reçu manuel) puisque tout est de toute façon illimité, en ne construisant le système de facturation qu'une fois trois clients réels et un prix validés. Rappel que MonCash dépend des credentials Digicel Business, qui bloquent KONEKTE depuis juillet.

**Sur « fais ce que tu juges nécessaire », périmètre volontairement restreint** à corriger le défaut introduit la veille et à fermer l'écart entre ce qui avait été affirmé et ce qui avait été prouvé, sans démarrer la facturation que je venais de déconseiller. Deux correctifs livrés (commit `7c04d78`) : (1) le rate limiting renvoyait le message par défaut de la librairie, **« ThrottlerException: Too Many Requests »**, que le frontend affiche tel quel, donc une exception technique en anglais au milieu d'une interface française — remplacé par un message qui indique surtout **quand** réessayer, calculé depuis `timeToExpire` ; (2) les 20 messages d'erreur du backend étaient écrits **sans accents** (« Cet email est deja utilise », « Role insuffisant ») alors que toute l'interface est correctement accentuée — corrigés, ainsi que les réponses du copilote et l'email d'invitation, les commentaires du code restant volontairement sans accents conformément à la convention du projet.

**Première passe navigateur réelle du projet dans une session de revue** (Playwright installé dans le scratchpad, soumission programmatique des formulaires conformément à la limitation connue de cette machine sur les clics synthétiques) : connexion via l'UI réussie, **7 pages sur 7 en HTTP 200 avec zéro erreur console** (`/dashboard`, `/clients`, `/produits`, `/ventes`, `/copilote`, `/parametres`, `/parametres/utilisateurs`), et le message de limitation **capturé à l'écran** dans sa bannière d'erreur après la 11e tentative depuis un contexte vierge. Captures inspectées : le rendu est soigné et cohérent avec le design system teal/or. Encodage UTF-8 des accents confirmé sur le fil (409 renvoyant « Cet email est déjà utilisé » correctement accentué). `tsc --noEmit` propre, 10 tests e2e verts, base remise à son état initial, les deux serveurs arrêtés par PID, ports 4009 et 3011 libres.

**Branche `durcissement/rate-limit-audit-rls` désormais à 2 commits** (`bc6216e` puis `7c04d78`), toujours non fusionnée dans `master` et non poussée (dépôt local sans remote).

## 2026-08-12 (suite)

### REMED : Phases 3 (Ordonnances enrichies), 6 (Dashboard), 7 (Rapports), 8 (Notifications), 9 (Sécurité) livrées — REMED entièrement finalisé

**Contexte :** sur « PHASE RESTANTES » (sans réponse à la question de portée posée juste avant), décision assumée d'enchaîner sur toutes les phases encore ouvertes du `PLAN.md` (3, 6, 7, 8, 9), la Phase 10+ (tests/Docker/multi-devises) restant volontairement hors scope par choix déjà acté avec Jaslin.

**Phase 3 — Ordonnances enrichies :** refonte du modèle, passé d'une ordonnance possédée 1:1 par une vente (créée uniquement à la volée) à une entité **autonome** : `Ordonnance` (statut ENREGISTREE/PARTIELLEMENT_SERVIE/SERVIE/ANNULEE, pièce jointe) + `PrescriptionItem` (médicament, dosage, posologie, quantité prescrite/servie). Une ordonnance peut désormais être enregistrée à l'avance (le patient revient plus tard) et servie en plusieurs fois sur plusieurs ventes. `Vente.ordonnanceId` passé de relation 1:1 possédée à FK simple non-unique. Deux chemins au POS : sélectionner une ordonnance existante (avec garde stricte sur le reste prescrit par produit) ou en créer une nouvelle à la volée (UX inchangée depuis la Phase 1, entièrement servie dans la foulée). **Annulation de vente symétrique** : si la vente était rattachée à une ordonnance, la délivrance est défaite (quantité servie décrémentée, statut recalculé) sinon l'ordonnance resterait faussement "servie" après un retour en stock. Upload de pièce jointe (image/PDF, multer, stockage disque local hors `src/`, servi via `/uploads`). Nouveau module `/ordonnances` (page dédiée) + sélecteur "nouvelle/existante" ajouté au panneau ordonnance de `/ventes`.

**Phase 6 — Dashboard enrichi :** CA/bénéfice estimé/dépenses/ruptures sur période filtrable (jour/semaine/mois/année), graphiques SVG maison (skill `dataviz` chargée avant codage — décision cohérente avec OTELA : pas de nouvelle librairie de charts, tokens de couleur du design system teal réutilisés) : évolution du CA (ligne avec crosshair), produits les plus vendus (barres), dépenses par catégorie (barre de proportion + légende).

**Phase 7 — Rapports :** page à onglets Ventes/Stock/Achats/Finance avec export CSV (générateur maison, BOM UTF-8, même philosophie qu'ASSOCOTISE — pas d'Excel/PDF, aucun besoin exprimé au-delà de la facture 80mm déjà livrée).

**Phase 8 — Notifications :** table `Notification` formalisant les alertes stock bas/péremption déjà calculées, cloche dans le header avec badge. Pas de scheduler (aucun projet du portefeuille n'en a) : synchronisation paresseuse à la lecture. **Bug réel trouvé et corrigé en vérification** : la déduplication initiale ne regardait que les notifications non lues — cliquer "tout marquer lu" sur une alerte encore vraie (produit toujours sous son seuil) la faisait recréer instantanément par le prochain appel de synchronisation, annulant silencieusement l'action de l'utilisateur. Corrigé en dédupliquant sur une fenêtre de 24h plutôt que sur le statut lu/non lu ; revérifié : le compteur reste à 0 après "tout marquer lu" même en rappelant l'endpoint plusieurs fois.

**Phase 9 — Sécurité & Audit renforcés :** verrouillage de compte après 5 échecs de connexion (30 min), au-delà du rate limiting déjà en place (qui protège une IP, pas un compte précis) — testé réellement (5 tentatives → 6e refusée même avec le bon mot de passe), tracé dans `AuditLog`. Audit d'écart trouvé et corrigé : la création de catégorie de produit n'appelait pas `logAudit` (seul trou trouvé, le reste du portefeuille était déjà correctement couvert). Revue RBAC : matrice cohérente confirmée sans autre écart.

**Incident d'outillage découvert et corrigé pendant la vérification** : un `req.query as {...}` avec un champ `boolean` (`notification.controller.ts`) compilait silencieusement sans erreur signalée par un `tsc --noEmit` en arrière-plan (complété avec l'exit code 0 rapporté par l'outil), mais faisait planter `ts-node` au démarrage réel du serveur avec une vraie erreur TS2352 ("insufficient overlap"). Corrigé (cast via `unknown` d'abord, même pattern que les casts `req.query` ailleurs dans le portefeuille). **Leçon retenue** : un `tsc --noEmit` réussi en arrière-plan ne remplace pas le démarrage réel du serveur comme vérification finale — les deux ont été faits systématiquement après ce constat.

**Migrations Prisma (5 au total sur cette suite de phases : ordonnances enrichies, notifications, verrouillage de compte)** : même limitation d'environnement déjà documentée (le moteur de schéma Prisma refuse `migrate dev`/`--create-only` en non-interactif dès qu'un avertissement de contrainte apparaît, ou reste bloqué sans avertissement) — contournée à chaque fois en écrivant la migration SQL à la main puis en l'appliquant via un script Node ponctuel (`pg`, installé puis désinstallé à chaque fois), suivi de `prisma generate` (moteur de requête, non concerné par la limitation).

**Vérifié en conditions réelles** : `tsc --noEmit` **et** démarrage réel du serveur (les deux, après l'incident ci-dessus) propres backend et frontend. API (curl) : cycle complet ordonnance (création standalone → vente partielle 8/20 → sur-service refusé → annulation restituant l'ordonnance à ENREGISTREE → vente inline classique → vente sans ordonnance refusée), pièce jointe uploadée et servie statiquement (200), RBAC 403 VENDEUR sur création d'ordonnance, rapports peuplés avec une vraie vente (CA/produits cohérents), export CSV téléchargé et vérifié, notifications synchronisées avec la péremption Amoxicilline réelle du seed, verrouillage de compte testé avec 6 tentatives réelles. Navigateur réel (Browser pane) : dashboard avec les 3 graphiques rendus (ligne CA, barres produits, proportion dépenses), page Rapports avec données réelles affichées, page Ordonnances, dropdown de notifications avec le contenu réel. Base remise à l'état seed propre (`prisma migrate reset` + reseed), fichier de test uploadé supprimé, processus Node zombies (limitation Windows déjà documentée) identifiés et arrêtés par PID.

**REMED est désormais fonctionnellement complet sur les 9 phases du plan fusionné.** Reste hors scope, documenté et assumé : Phase 10+ (tests automatisés, Docker, multi-devises — jamais demandés explicitement). Jamais commité en git au moment de l'écriture de cette entrée.

### REMED : gestion des utilisateurs livrée (trou hors phasage repéré par Jaslin)

**Contexte :** sur « qu'en est-il de la gestion des utilisateurs ? », vérification immédiate confirmant un vrai manque — REMED avait 5 rôles et un verrouillage de compte (Phase 9) mais aucune interface pour qu'un GERANT/SUPER_ADMIN crée un compte employé ; seul le script de seed ou un accès direct à la base le permettait. Jamais mentionné dans les 10 phases du plan fusionné initial.

**Livré :** module `/api/utilisateurs` réservé SUPER_ADMIN/GERANT (`requireAdmin`) — liste, création (mot de passe initial fixé par l'admin, pas d'invitation email faute de SMTP configuré, même choix qu'OTELA), modification (rôle/téléphone/nom/prénom/statut), réinitialisation de mot de passe. Page `/utilisateurs` (visible seulement pour SUPER_ADMIN/GERANT dans la sidebar, même filtre côté client que le backend). Trois gardes de sécurité, toutes testées réellement : anti-escalade de privilèges (seul un SUPER_ADMIN peut créer/promouvoir un SUPER_ADMIN — un GERANT a reçu un 403 réel en testant), anti-auto-verrouillage (un compte ne peut ni changer son propre rôle ni se désactiver, testé sur le compte admin lui-même), garde du dernier SUPER_ADMIN actif. Désactivation et reset de mot de passe révoquent immédiatement les sessions actives (suppression des refresh tokens) — vérifié qu'un compte désactivé ne peut plus se reconnecter, et qu'un reset fonctionne avec le nouveau mot de passe.

**Vérifié en conditions réelles** : backend démarré réellement (pas seulement `tsc --noEmit`, leçon retenue de la Phase 9 précédente) — création, doublon email 409, escalade de privilèges 403, auto-modification 400, désactivation immédiate 401 au login suivant, reset mot de passe avec connexion réussie au nouveau mot de passe, RBAC 403 pour un VENDEUR. Navigateur réel (Browser pane) : page `/utilisateurs` rendue avec les 4 comptes réels et leurs statuts corrects. Base remise à l'état seed, processus Node confirmés absents par PID après arrêt. `tsc --noEmit` propre des deux côtés. Jamais commité en git.

---

## 2026-08-12

### REMED : Phases 2 (Achats), Dépenses/Retours (Phase 4), Inventaire (Phase 5) livrées + facture 80mm

**Contexte :** sur « finaliser remed. pour les facture je les veux 80mm », clarification de portée proposée (juste la facture / toutes les phases restantes / un sous-ensemble) restée sans réponse — décision assumée d'enchaîner sur les phases restantes du `PLAN.md` (2, 4, 5) en plus de la facture explicitement demandée, en documentant clairement ce qui n'a pas été traité plutôt que de deviner un périmètre plus large.

**Incident d'environnement notable, résolu** : `prisma migrate dev` (et même `migrate diff`/`db execute`/`validate`) restaient bloqués indéfiniment dans cet environnement — pas la limitation de confirmation interactive déjà documentée sur REMED Phase 0.5/1, mais le moteur de schéma Prisma (JSON-RPC over stdio) qui ne répond jamais une fois lancé via `npx prisma`. Diagnostiqué en testant le binaire `schema-engine-windows.exe` directement (`--help` répond instantanément, donc le binaire n'est pas en cause, seul le mode serveur RPC l'est). Une tentative de migration en arrière-plan a en réalité fini par aboutir silencieusement avant d'être arrêtée par `TaskStop` (aucune sortie n'avait encore été affichée) — découvert en inspectant directement la base (tables et enums déjà créés, `_prisma_migrations` déjà à jour) après avoir écrit et commencé à appliquer une migration de secours à la main. La migration écrite manuellement (redondante) a été supprimée, celle générée par Prisma conservée. `prisma generate` (moteur de requête, non concerné) fonctionne normalement.

**Phase 2 — Achats/Fournisseurs** : `CommandeAchat`/`LigneCommandeAchat` (déjà présents au schéma depuis la Phase 0) enfin exposés via un module complet (brouillon → envoyée → reçue partielle/complète), réception ligne par ligne réutilisant `stock.service.entree` (numéro de lot + date de péremption par ligne, cumul si le même lot revient). Page `/achats`.

**Phase 4 (partielle) — Dépenses et Retours** : nouveaux modèles `Depense` (catégorie, montant, mode de paiement — une dépense en espèces exige une caisse ouverte et pousse automatiquement une `CaisseTransaction SORTIE_MANUELLE`, pas de double comptabilité) et `Retour`/`RetourItem` (RETOUR_CLIENT/RETOUR_FOURNISSEUR/PRODUIT_ENDOMMAGE/PRODUIT_EXPIRE/ERREUR_VENTE, nouveau `TypeMouvementStock.RETOUR_CLIENT` symétrique de `RETOUR_FOURNISSEUR`, sens du mouvement dérivé du type jamais saisi côté client, sorties protégées par compare-and-swap). Pages `/depenses` et `/retours`. Le rapport de fermeture de caisse formalisé et le paiement fournisseur (Phase 2) restent non traités, pas de besoin exprimé.

**Phase 5 — Inventaire** : `Inventaire`/`InventaireItem` (complet ou partiel, quantité théorique figée à la création, validation réservée aux rôles de gestion générant les `AJUSTEMENT_POSITIF`/`AJUSTEMENT_NEGATIF` via `stock.service.ajuster`). Page `/inventaire` avec écart calculé en direct.

**Facture/reçu PDF 80mm (demande explicite)** : `pdfkit` ajouté, ticket thermique 80mm exact avec hauteur calculée dynamiquement au contenu (rendu une première fois dans un document jetable pour mesurer la hauteur finale, PDFKit ne permettant pas de redimensionner une page déjà créée). `GET /api/ventes/:id/facture.pdf`, bouton d'impression sur le reçu de vente et icône imprimante sur chaque ligne de l'historique des ventes.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. API (curl) : cycle achat complet avec sur-réception refusée, garde caisse-fermée sur dépense espèces avec solde théorique exact à la fermeture, retour client (+5) et produit endommagé (-3) sur le même lot avec sortie excessive refusée, inventaire avec validation bloquée tant que la saisie n'est pas complète puis ajustement de stock appliqué et vérifié, RBAC 403 confirmé pour un VENDEUR sur dépenses/retours/validation d'inventaire. Facture PDF téléchargée et inspectée : en-tête `%PDF-1.3` valide, `MediaBox` confirmant exactement 80mm de large. Navigateur réel (Browser pane) : les 4 nouvelles pages rendues et fonctionnelles avec les données créées au curl. Base remise à l'état seed propre (`prisma migrate reset` + reseed) après vérification, processus Node zombies de nodemon (limitation Windows déjà documentée ailleurs) identifiés et arrêtés par PID.

**Reste à faire** (documenté dans `PLAN.md`, non traité cette session) : Phase 3 (upgrade Ordonnances avec statut de délivrance et pièce jointe — le modèle simple actuel suffit à bloquer une vente sans ordonnance), Phase 6 (dashboard graphique), Phase 7 (rapports exportables), Phase 8 (notifications persistées), Phase 9 (sécurité/audit renforcés), Phase 10+ (tests automatisés, Docker, multi-devises — toujours repoussés par choix explicite). Jamais commité en git au moment de l'écriture de cette entrée.

## 2026-08-10

### REMED : nouveau projet de gestion de pharmacie, Phase 0 (socle) livrée

**Contexte :** sur "une application pour la gestion d'une pharmacie", clarification avant de coder (AskUserQuestion) : SaaS générique sans client identifié, périmètre Vente/POS + Stock + Achats/Fournisseurs + Ordonnances + Rapports, même stack que le reste du portefeuille (Next.js App Router + Express 4 + TypeScript + Prisma + PostgreSQL). Nouveau projet `livrables/applications/REMED/`, ports 4011 (backend) / 3013 (frontend — 3012 déjà pris par `portfolio-jaslin` dans `.claude/launch.json`, corrigé avant de démarrer les serveurs). Phasage écrit dans `PLAN.md` du projet avant de coder.

**Livré (Phase 0 — socle) :** auth JWT complète calquée sur le pattern POSTA (access 15 min + refresh 30j rotatif à chaque appel, cookie httpOnly, hash SHA-256 en base) + RBAC 5 rôles (SUPER_ADMIN, GERANT, PHARMACIEN, VENDEUR, MAGASINIER). Schéma Prisma couvrant tout le domaine dès le départ (produits, lots avec numéro de lot + date de péremption, mouvements de stock append-only, fournisseurs, commandes d'achat, ventes, ordonnances, session de caisse, journal d'audit) — seuls Produits/Stock/Fournisseurs/Dashboard ont une API en Phase 0, le reste du schéma est posé pour les phases suivantes. Décision de conception centrale : la quantité en stock d'un produit n'est jamais stockée directement, toujours recalculée comme la somme des lots actifs, pour ne jamais pouvoir diverger de la traçabilité par lot (péremption). Ajustements de stock protégés par compare-and-swap (même pattern que BANKA/GESCOM/SHOPAY contre la concurrence). Frontend Next.js avec un design system teal clinique dédié, dérivé du design system GESCOM/ANTENN (Badge/StatCard/Modal/EmptyState/PageToolbar réutilisés à l'identique, seuls les tokens de couleur changent).

**Incident mineur résolu avant démarrage** : le port frontend initialement choisi (3012) entrait en conflit avec `portfolio-jaslin` déjà déclaré dans `.claude/launch.json` — détecté par relecture du fichier avant de lancer les serveurs, basculé sur 3013 (backend `.env`/`.env.example` et `package.json` corrigés en conséquence) avant tout démarrage réel, aucun conflit rencontré en pratique.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. API testée au curl sur la vraie base Postgres locale (`remed`) : login, `/me`, refresh avec rotation confirmée, logout puis `/me` → 401, RBAC (un compte VENDEUR tentant de créer un produit → 403), entrée de stock créant un lot + mouvement, liste des lots. Navigateur réel (Playwright) : connexion via l'UI avec le compte de démo, dashboard avec KPI réels (valeur du stock, alerte péremption sur le lot Amoxicilline), page Produits (quantité recalculée correctement après l'entrée de stock testée en curl), page Stock (mouvement affiché), page Fournisseurs. Données de test (lot `LOT-PARA-002` créé pendant la vérification API) supprimées après coup, base remise à l'état seed. Serveurs arrêtés proprement en fin de session. Comptes seed : `admin@remed.ht` / `ChangeMoi123!` (SUPER_ADMIN), `pharmacien@remed.ht` / `Pharmacien123!` (PHARMACIEN), `vendeur@remed.ht` / `Vendeur123!` (VENDEUR).

**Reste à faire** (documenté dans `PLAN.md`, hors périmètre de cette session) : Phase 1 Ventes/POS (décrément FEFO du stock via les lots, session de caisse, reçu), Phase 2 Achats/Fournisseurs (commande → réception ligne par ligne réutilisant le service d'entrée de stock), Phase 3 Ordonnances (saisie rattachée à une vente contenant un produit à prescription obligatoire), Phase 4 Rapports (ventes/marge, valorisation et rotation du stock, achats). Jamais commité en git au moment de l'écriture de cette entrée.

### REMED : cahier des charges élargi reçu, conflit de stack tranché, Phase 0.5 multi-pharmacie livrée

**Contexte :** Jaslin a fourni un prompt très détaillé (46 sections) demandant une refonte complète — stack NestJS + MySQL + React/Vite/shadcn, architecture multi-pharmacie, RBAC granulaire — en conflit direct avec la Phase 0 de REMED tout juste livrée sur Express/PostgreSQL/Next.js. Plutôt que d'exécuter aveuglément, conflit signalé explicitement (tableau comparatif) et clarifié (AskUserQuestion) : **continuer sur la stack déjà construite et vérifiée**, le cahier des charges devenant la source fonctionnelle enrichie à fusionner dans les phases suivantes plutôt qu'un redémarrage.

**Cahier des charges fusionné dans `PLAN.md`** avec décisions d'architecture explicites et justifiées : RBAC en rôles enum fixes plutôt qu'un moteur de permissions granulaire (déjà la décision prise sur NEXORA dans ce même portefeuille) ; multi-pharmacie ajouté tout de suite plutôt que reporté (retrofit plus coûteux plus tard) ; tests automatisés et Docker repoussés et signalés explicitement (aucun projet du portefeuille n'a de tests automatisés à ce jour, écart d'effort à trancher par Jaslin le moment venu) ; multi-devises repoussé après le cœur métier. Phasage révisé en 10 phases. Sur « vas y », Phase 0.5 (multi-pharmacie) exécutée en premier.

**Livré (Phase 0.5) :** modèle `Pharmacie` (nom, adresse, devise, préfixe facture, seuils par défaut). `pharmacieId` ajouté sur les tables propriétaires (`Utilisateur`, `Categorie`, `Produit`, `Fournisseur`, `CommandeAchat`, `Vente`, `CaisseSession`), résolu uniquement depuis le JWT vérifié côté serveur — jamais un paramètre client, même garde que `resolveBoutique` sur SHOPAY. Les tables enfant (lots, mouvements, lignes) héritent du scope via leur parent sans colonne redondante, même principe que SHOPAY. Contraintes d'unicité (nom de catégorie, code-barres, numéros de vente/commande) repensées par pharmacie plutôt que globales.

**Incident technique résolu** : `prisma migrate dev` refusait de s'exécuter dans cet environnement non-interactif dès qu'une confirmation de perte de données potentielle était requise (`Error: ... non-interactive, which is not supported`), y compris en pipant une confirmation — limitation dure de l'outil, pas contournable en arrière-plan non plus. Résolu en supprimant entièrement la base via `prisma db execute` (connexion à la base `postgres` de maintenance, `DROP DATABASE IF EXISTS remed`) puis en rejouant une migration initiale fraîche sur base vide (aucune donnée réelle existante, seed re-rejouée après coup).

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. API re-testée au curl (login incluant désormais `pharmacieId` dans le JWT, produits/fournisseurs/catégories/stock/dashboard tous scopés et cohérents). **Test d'isolation cross-tenant qui mord réellement** : une deuxième pharmacie de test créée avec son propre compte admin — liste de produits et fournisseurs vide pour cette pharmacie, lecture d'un produit de l'autre pharmacie par ID → 404, tentative d'ajustement de stock sur un lot d'une autre pharmacie → 404, dashboard à zéro. Pharmacie et compte de test supprimés après vérification. Navigateur réel (Playwright) : login et dashboard fonctionnels après la migration, données scopées correctement affichées. Serveurs arrêtés proprement en fin de session.

**Reste à faire** : Phase 1 Ventes/POS (prochaine étape naturelle), puis Achats, Ordonnances enrichies, Caisse & Finance, Inventaire, Dashboard enrichi, Rapports, Notifications, Sécurité renforcée — détail complet dans `PLAN.md`. Jamais commité en git au moment de l'écriture de cette entrée.

### REMED : Phase 1 (Ventes / Point de vente) livrée

**Contexte :** sur « continue », enchaînement direct sur la Phase 1 du plan fusionné (POS complet). Modèles ajoutés au schéma : `Client`, `Paiement` (paiement scindé, réutilisable pour achats/dépenses futurs), `CaisseTransaction` (mouvements de caisse append-only), `Vente` enrichie (`clientId`, `caisseSessionId` obligatoire, `sousTotal`/`remise`/`montantTotal`, champs d'annulation tracés). Nouvel enum `TypeMouvementStock.ANNULATION_VENTE`.

**Livré :** module Clients (CRUD + historique des ventes sur la fiche) ; module Caisse (`/caisse/ouvrir`, `/:id/fermer`, `/mouvements`, `/active`, `/historique` — une seule session ouverte par pharmacie à la fois, fermeture calculant `soldeTheorique`/`ecart` à partir des seules transactions ESPECES) ; module Ventes avec **FEFO réellement implémenté et testé avec répartition sur plusieurs lots** (`consommerFefo`, compare-and-swap par lot comme l'ajustement de stock), **blocage effectif de la vente sur un produit à ordonnance sans ordonnance fournie**, **paiement scindé** (plusieurs modes, somme validée serveur), **annulation de vente** restituant précisément la quantité sur le lot d'origine avec mouvement `ANNULATION_VENTE` et contre-passation de la transaction de caisse, réservée aux rôles de gestion (pas au vendeur). Frontend : page `/ventes` complète (recherche produit type scanner, panier avec quantités, sélection client, remise, bloc ordonnance conditionnel, paiement scindé avec total contrôlé en direct, ventes récentes avec bouton d'annulation selon rôle, reçu affiché après vente), page `/clients` (liste + fiche avec historique), navigation mise à jour.

**Incident d'environnement résolu** : `prisma migrate dev` refuse systématiquement de s'exécuter dans cet environnement dès qu'une confirmation de perte de données potentielle est requise (même limitation déjà rencontrée en Phase 0.5, y compris en arrière-plan et en pipant une confirmation). Contourné à chaque évolution de schéma de cette phase en supprimant la base via `prisma db execute` (`DROP DATABASE`) puis en rejouant une migration initiale fraîche sur base vide — sans perte, base toujours en seed à ce stade du projet.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. Cycle complet au curl : vente bloquée (400) sans ordonnance sur un produit qui en nécessite une, acceptée avec ordonnance (stock du bon lot décrémenté 40→35), paiement scindé espèces+carte accepté, vente rejetée (400) si la somme des paiements ne correspond pas au total, RBAC vérifié (VENDEUR peut vendre mais reçoit 403 sur l'annulation), annulation restituant exactement le stock (35→40) avec double-annulation refusée (400), fermeture de caisse avec écart exact (vérifié à la transaction près, y compris la contre-passation de l'annulation), blocage de toute vente hors session de caisse ouverte. **Navigateur réel (Playwright)** : ouverture de caisse depuis l'UI, recherche produit avec badge « Ordonnance », panier, formulaire d'ordonnance apparu automatiquement, total calculé en direct, finalisation, reçu affiché à l'écran, stock recalculé visible sur `/produits` (40→39 après la vente réelle passée par le navigateur), fermeture de caisse ramenant l'écran à l'état bloquant attendu. Base remise à l'état seed propre après chaque vérification (reset + reseed), serveurs arrêtés proprement.

**Reste à faire** : facture PDF imprimable (reçu écran suffisant pour l'instant), puis Phase 2 Achats/Fournisseurs, Phase 3 Ordonnances enrichies (pièce jointe, service partiel), Phase 4 Caisse & Finance (dépenses, retours), Phase 5 Inventaire, Phase 6-9 Dashboard/Rapports/Notifications/Sécurité — détail dans `PLAN.md`. Jamais commité en git au moment de l'écriture de cette entrée.

## 2026-08-09

### NEXORA : gestion des utilisateurs (P1) — le plan MVP est désormais entièrement clos

**Contexte :** sur "on continue le développement de nexora", audit du plan MVP révélant que "Gestion des utilisateurs" (invitation, rôles, permissions) était le seul point encore non traité — `register` ne créait que le premier ADMIN, aucun moyen d'inviter un second utilisateur dans une organisation existante.

**Décision (AskUserQuestion)** : invitation par lien à usage unique envoyé par email (plutôt qu'un mot de passe initial fixé par l'ADMIN façon OTELA).

**Livré :** modèle `Invitation` (token opaque haché SHA-256, RLS) ; lecture cross-tenant du token via `AuthPrismaService` (BYPASSRLS), même principe que le login par email, la création du compte repassant par le client scopé au tenant comme `register` ; `MailService` (nodemailer, repli en journal si SMTP absent, même philosophie que POSTA/ASSOCOTISE) ; anti-lockout complet (un ADMIN ne peut ni changer son propre rôle ni se désactiver, aucune action ne peut faire tomber le nombre d'ADMIN actifs à zéro, vérifié y compris contre un token JWT encore valide d'un compte déjà désactivé en base) ; désactivation d'un compte révoque ses refresh tokens actifs ; page `/parametres/utilisateurs` (ADMIN) et `/accepter-invitation/[token]` (publique, émet une session comme après un register).

**Incident d'environnement résolu** : après une coupure ayant arrêté les deux serveurs de dev, le frontend relancé servait un faux 404 sur la nouvelle route — cache `.next` périmé du 27/07 (même cause déjà documentée sur ce projet), résolu en le supprimant avant de relancer.

**Vérifié en conditions réelles** : cycle complet API (curl) et navigateur (Playwright, captures d'écran) — invitation, repli SMTP avec lien correct, acceptation avec émission de session, réutilisation de token refusée, RBAC, auto-modification refusée, promotion de rôle, garde du dernier ADMIN, révocation d'invitation, révocation de session à la désactivation. `tsc --noEmit` propre. Commité (`0463119`). **Tous les P0 et P1 du plan MVP NEXORA sont désormais réellement et intégralement livrés.**

### GESCOM : bilan comptable déséquilibré vérifié résolu

**Contexte :** sur question de Jaslin ("qu'est-ce qu'il reste à faire pour compléter GESCOM"), reprise du dernier point ouvert signalé fin juillet (Actif 30 141,2 HTG ≠ Passif 27 616,2 HTG, cause jamais confirmée).

**Investigation :** relecture de `compta.service.ts::getBilan()` — le correctif déjà en place (solde signé au lieu de `Math.abs()`, résultat de l'exercice intégré au passif) est algébriquement toujours équilibré tant que chaque écriture reste une paire débit/crédit valide sur les 4 types de comptes (ACTIF/PASSIF/PRODUIT/CHARGE), ce qui est garanti par les FK Prisma. Diagnostic empirique plutôt que théorique : script de lecture direct sur la vraie base (13 écritures) + appel réel de `getBilan()`.

**Résultat :** `equilibre: true`, Actif = Passif = 30 041,2 HTG. La somme de tous les soldes de tous les comptes vaut exactement 0 (identité comptable respectée). L'écart signalé en juillet avait déjà été corrigé par une écriture manuelle du 2026-08-03 trouvée dans le journal : *"Correction : annulation VNT-000003 jamais contrepassée avant le fix du 2026-08-03"* — Jaslin avait lui-même trouvé et corrigé, en parallèle dans son IDE, une vente annulée dont la contre-passation comptable n'avait jamais été enregistrée.

**Aucun changement de code nécessaire** pour le bilan — vérification uniquement. Scripts de diagnostic temporaires créés dans `backend/` puis supprimés après usage.

**Correction annexe (même session) :** nom de fournisseur mal encodé en base corrigé — "Distributeur Cara?be" → "Distributeur Caraïbe" (`Fournisseur.nom`, `id: cmr116un4000036ixcmteouu2`), simple `UPDATE` via script ts-node ponctuel, supprimé après usage.

**Reste sur GESCOM** : tests automatisés (toujours zéro sur le projet). Machine chargée en fin de session (plusieurs process Node zombies accumulés, ralentissant ts-node) — pattern déjà documenté ailleurs sur ce portefeuille.

## 2026-08-07 (suite)

### UJEPH (etudiant-universite-web) : migration complète Supabase → Express/Prisma/MySQL + contenu réel du site actuel

**Contexte :** le site d'admissions UJEPH tournait entièrement sur Supabase Cloud (gratuit, mais Jaslin voulait éviter un futur coût d'hébergement séparé et unifier avec le serveur MySQL déjà utilisé par ACADÉMIE). Décision prise et reconfirmée explicitement par Jaslin en cours de session malgré l'ampleur du chantier. Plan en 6 phases validé via `ExitPlanMode`, exécuté d'affilée sans pause (« enchaîner sans s'arrêter »).

**Livré (migration technique)** : nouveau backend `backend/` (Express + TypeScript + Prisma + MySQL, port 4009), calqué sur les patterns POSTA (auth JWT, refresh haché+rotation, validate.middleware corrigé) et LAKAY (Socket.io). Rôle unique `Role` (STUDENT/FACULTY/ADMIN/SUPER_ADMIN) remplaçant les deux systèmes de rôles Supabase qui se chevauchaient. Phase 0-1 : auth + contenu public (programmes/cursus, actualités, témoignages, FAQ, contact, calendrier). Phase 2 : candidatures + upload documents (disque local, téléchargement protégé) + paiement MonCash/NatCash (preuve manuelle, config admin dynamique). Phase 3 : devoirs/rappels/notifications (REST+polling). Phase 4 : messagerie et présence temps réel via Socket.IO (handshake par cookie httpOnly, pas de token JS lisible) + forum. Phase 5 : suppression de `@supabase/supabase-js` et `src/integrations/supabase`, migrations Postgres historiques déplacées vers `docs/supabase-legacy-schema/`. Chaque phase vérifiée par tsc/build + tests fonctionnels réels (curl, double connexion Socket.IO simultanée pour confirmer la diffusion temps réel, vérification RBAC/ownership).

**Au passage** : suppression de composants à données entièrement fabriquées préexistants (`NotificationPanel` mort jamais branché, `useOnlineStatus` remplacé par un `usePresence` unifié), correction d'un bug préexistant dans `validate.middleware` qui ignorait silencieusement le résultat du parsing Zod.

**Livré (contenu réel, sur demande explicite de Jaslin après la migration)** : visite complète de ujeph.edu.ht pour en extraire le contenu réel — description/mission/doyen/approche éducative/domaines d'expertise pour les 5 facultés (ajout des champs `deanName`/`highlights` au modèle Program), nouvelle section « Pourquoi Choisir l'UJEPH » sur l'accueil (mission/vision/5 piliers repris de la page « À propos »), remplacement des photos de stock Unsplash du hero par de vraies photos UJEPH (étudiants sur le campus, équipe de football UJEPH CALCIO), photos réelles associées aux 2 actualités déjà en base. 2 photos écartées du lot téléchargé (portrait d'un individu non identifié, photo de mode sans rapport avec l'université) car sans lien établi avec UJEPH ou représentant un particulier hors contexte institutionnel — pas utilisées par principe de ne jamais publier une personne sans rapport clair. **Événements réels : vérifié qu'aucun n'existe** — le lien « Événements » du menu du site actuel pointe vers la démonstration du thème WordPress Eduma, jamais personnalisée ; rien fabriqué pour combler ce vide.

**Vérifié en conditions réelles** : tsc/build propres backend et frontend à chaque étape. Capture Playwright de l'accueil et de la page facultés confirmant le rendu (photos réelles, section piliers, détail par faculté). Travail commité en 2 commits sur la branche `rebranding-ujeph-securite-paiement` (non mergée dans `main`) et poussé sur le remote GitHub.

**Reste, non traité** : merge de la branche dans `main` (pas demandé), déploiement en production, migration des données de test/uploads locaux si un vrai lancement suit.

### SMART GROS-MORNE : Phase 4 (bibliothèque de fichiers générique) livrée

**Contexte :** quatrième "vas y" consécutif sans réponse à la question de priorité posée en début de journée précédente. Choix assumé, même critère que les fois précédentes : sur les items restants de Phase 4 (commentaires/modération, notifications internes, gestion de menus dynamique, sauvegardes UI, 2FA/liste noire IP/CAPTCHA), la bibliothèque de fichiers générique est la seule constructible sans identifiant externe manquant ni décision UX imposée à l'admin unique.

**Livré :** modèle `Document` (titre, description optionnelle, `mediaId` réutilisant le mécanisme `Media` déjà en place — l'upload PDF était déjà autorisé par `upload.middleware.ts` depuis le début du projet, aucun nouveau stockage nécessaire). Pas de table de traduction : un document officiel est déjà rédigé dans sa langue, contrairement au contenu narratif du reste du site. Module backend complet (`documents`, public/admin, RBAC identique aux autres modules). Page publique `/documents` (liste téléchargeable, taille de fichier formatée, icône par type). Écran admin `/admin/documents` (upload direct ou réutilisation d'un média déjà en bibliothèque, même pattern que Témoignages/Partenaires). Lien ajouté au Footer ("Informations utiles", entre FAQ et Mentions légales) et au `sitemap.ts` (déjà construit en Phase 5 la veille) ; clé `documents` ajoutée à la liste des 24 pages de l'admin Images de fond pour rester cohérent.

**Deux incidents opérationnels rencontrés et résolus, aucun lié au code livré** : (1) la première tentative de migration Prisma a échoué deux fois de suite (`Error: UNKNOWN: unknown error` puis `P1002` timeout sur le verrou consultatif Postgres) à cause de plusieurs processus `prisma migrate dev` restés vivants en arrière-plan d'une tentative précédente — identifiés via `Get-CimInstance Win32_Process`, arrêtés, la migration a ensuite réussi du premier coup. (2) Au redémarrage du backend pour la vérification, le serveur a mis plus de deux minutes à répondre à `curl` — cause réelle : une charge CPU à 100% due à l'accumulation de plusieurs instances `npm run dev`/`nodemon` du backend restées vivantes au fil de la journée (même limitation Windows déjà documentée à plusieurs reprises) **combinée à un projet sans rapport** (`etudiant-universite-web`, Vite) tournant en parallèle sur la même machine, jamais touché. Une fois les instances backend dupliquées nettoyées, un unique redémarrage a fonctionné normalement — le serveur avait en réalité fini par démarrer correctement pendant l'attente, seuls les contrôles `curl` à délai court (2s) échouaient à cause de la latence induite par la charge machine.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. API (curl — cycle complet création/lecture publique/lecture admin/suppression, avec un vrai fichier déjà en bibliothèque média). Navigateur : page publique `/documents` rendue avec le document de test (titre, description, taille "32 Ko" affichée correctement), écran admin `/admin/documents` listant correctement l'entrée (titre, nom de fichier original, statut, ordre), aucune erreur console. Donnée de test nettoyée après vérification, serveurs arrêtés proprement.

**Reste, non traité** : commentaires + modération, notifications internes, gestion de menus dynamique, sauvegardes UI, sécurité avancée (2FA, liste noire IP, CAPTCHA — ce dernier nécessite toujours une clé tierce indisponible). Reste de la Phase 5 (i18n réel, performance) également non traité. Hors scope tant que Jaslin ne précise pas la priorité.

## 2026-08-06 (suite 3)

### SMART GROS-MORNE : Phase 4 (envoi réel de newsletter, volet partiel) livrée

**Contexte :** troisième "vas y" consécutif sans réponse à la clarification sur la priorité entre newsletter SMTP / SEO+i18n / sécurité avancée. Interprété comme un feu vert pour trancher par jugement propre plutôt que d'insister. Choix motivé : sur les items restants de Phase 4 (commentaires, notifications, bibliothèque de fichiers, menus dynamiques, sauvegardes, 2FA/CAPTCHA/liste noire IP), l'envoi réel de newsletter est le seul constructible sans identifiant externe déjà manquant dans cet environnement (CAPTCHA exige une clé tierce ; 2FA aurait changé l'UX de connexion de l'unique compte admin sans que Jaslin l'ait demandé).

**Livré :** `backend/src/utils/mailer.ts` (nodemailer, nouvellement installé) avec **repli gracieux** si la configuration SMTP est incomplète — 5 variables d'environnement optionnelles (`SMTP_HOST/PORT/USER/PASS/FROM`) ajoutées à `env.ts`, même philosophie que POSTA (mot de passe oublié) : le contenu est journalisé côté serveur plutôt que perdu, jamais d'échec côté utilisateur. Envoi individuel par abonné (pas de champ "À" groupé — évite d'exposer la liste des abonnés entre eux si un client mail l'affiche), échecs individuels non bloquants (le compte final reflète les envois réellement réussis). Nouvel endpoint `POST /api/newsletter/envoyer` (sujet + message, réservé admin) — **aucun modèle "Campagne" persisté**, décision assumée pour rester fidèle à la limite déjà posée en Phase 2f ("pas d'envoi de campagne planifiée/gabarits", cf. `PLAN.md`) : Phase 4 rend l'envoi réellement possible, elle ne construit pas un moteur de campagnes. L'action est déjà journalisée automatiquement par le middleware d'activité générique de la Phase 2f, sans aucun code supplémentaire. Écran admin `/admin/newsletter` enrichi d'un bouton "Envoyer un message" (modal sujet+message, désactivé si zéro abonné actif), confirmation native avant envoi (action irréversible), message de résultat honnête selon le mode réel (envoi effectif vs. journalisation).

**Incident opérationnel rencontré et corrigé en vérification** : plusieurs instances du backend (`npm run dev`/`nodemon`/`ts-node`) s'étaient accumulées au fil de la journée sans être toutes arrêtées malgré les précautions déjà prises lors des phases précédentes — un `npm run dev` a échoué en `EADDRINUSE` pendant qu'une instance plus ancienne, toujours vivante et déjà à jour avec le nouveau code, continuait de répondre. Identifié via `Get-CimInstance Win32_Process` (7 processus Node liés au projet trouvés, contre 1 attendu), tous arrêtés puis une seule instance propre relancée. Confirme, une fois de plus, que `TaskStop`/`preview_stop` ne garantit pas toujours la fin de l'arbre de processus complet sous Windows avec nodemon (limitation déjà documentée en Phase 1).

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. API (curl — refus propre "Aucun abonné actif" si la liste est vide, envoi en mode no-op confirmé avec le **contenu réel** du message retrouvé dans les logs serveur, pas seulement un indicateur booléen). Navigateur : le formulaire admin s'ouvre, se remplit et déclenche l'appel correctement, mais la confirmation native `window.confirm()` avant l'envoi n'a pas pu être pilotée par l'outil de navigateur de cette session (aucune requête réseau observée après le clic, confirmé via `read_network_requests` — limitation d'outillage documentée ailleurs sur ce portefeuille, pas un bug côté produit). Le flux d'envoi complet reste donc validé au niveau API/serveur par curl, pas par un clic de bout en bout dans le navigateur. Donnée de test (abonné + message envoyé) nettoyée après vérification. Serveurs arrêtés proprement (7 processus Node fantômes confirmés éteints avant la fin de session).

**Reste, non traité** : commentaires + modération, notifications internes, bibliothèque de fichiers générique, gestion de menus dynamique, sauvegardes UI, sécurité avancée (2FA, liste noire IP, CAPTCHA — ce dernier nécessite une clé tierce indisponible). Reste de la Phase 5 (i18n réel, performance) toujours non traité également (cf. entrée précédente du jour). Hors scope tant que Jaslin ne précise pas la priorité.

## 2026-08-06 (suite 2)

### SMART GROS-MORNE : Phase 5 (SEO technique, volet partiel) livrée

**Contexte :** sur "vas y" après la Phase 3, sans que Jaslin ne réponde à la question de clarification posée (newsletter SMTP réelle / SEO+i18n / sécurité avancée — Phase 4/5 n'étant pas scopée en détail comme les phases précédentes, contrairement à sa préférence habituelle documentée pour la clarification avant d'exécuter). Un second "vas y" sans réponse a été interprété comme "arrête de demander, avance" — décision assumée de traiter le **SEO technique uniquement**, seul sous-chantier ne nécessitant ni identifiants externes (SMTP pour la newsletter, clé CAPTCHA) ni contenu Kreyòl à fabriquer (contrairement à l'i18n réel, resté non traité).

**Livré :** `app/robots.ts` et `app/sitemap.ts` (24 pages statiques + `actualites/[id]`/`personnalites/[id]` publiés, fetch server-side tolérant aux pannes backend). `app/layout.tsx` enrichi (`metadataBase`, Open Graph/Twitter Card par défaut) et **une erreur factuelle préexistante corrigée** : le repli metadata racine décrivait Gros-Morne comme "ville du Nord-Ouest d'Haïti" alors que la commune est dans le département de l'**Artibonite** (déjà correct sur les pages qui définissaient leur propre `metadata`, jamais sur ce repli racine). `generateMetadata` dynamique ajouté sur les deux routes de détail (`actualites/[id]`, `personnalites/[id]`) : chaque article et chaque personnalité partageaient jusqu'ici exactement le même titre générique ("Actualité — Gros-Morne Vil Mwen" pour tous les articles sans exception) — remplacé par un vrai titre/description/image Open Graph par fiche, obtenu par un fetch server-side sur l'API publique (repli générique conservé en cas de panne ou de fiche introuvable). JSON-LD `GovernmentOrganization` ajouté sur l'accueil, à partir de coordonnées GPS déjà publiées ailleurs sur le site (conversion DMS→décimal, aucune donnée inventée). Nouveau `lib/seo.ts` centralisant `SITE_URL` (repli `localhost:3010` explicite — **le site n'a jamais été déployé, aucun domaine réel n'existe encore**, pas de nom de domaine deviné) et les coordonnées de la commune.

**Vérifié en conditions réelles** : `tsc --noEmit` propre. Navigateur : `/robots.txt` et `/sitemap.xml` rendus et corrects. Titre d'onglet confirmé dynamique sur une fiche article réelle ("Inauguration du nouveau marché communal — Gros-Morne Vil Mwen", au lieu du générique) et une fiche personnalité ("Paul Prompt — Gros-Morne Vil Mwen"). Balises `og:title`/`og:description`/`og:type` lues via JavaScript et correctes pour l'article testé. JSON-LD de l'accueil lu via JavaScript, structure et valeurs confirmées. Aucune erreur console sur les pages vérifiées. Serveurs arrêtés proprement (même précaution PID zombie `nodemon` que les phases précédentes de la journée).

**Reste ouvert, explicitement non traité** : i18n réel (le sélecteur de langue HT dans la Navbar reste décoratif, aucune traduction Kreyòl du contenu statique n'existe) et performance (cache, optimisation mobile) — les deux volets restants de la Phase 5, nécessitant soit du vrai contenu Kreyòl à écrire avec Jaslin soit un profilage dédié, ni l'un ni l'autre entrepris sans confirmation explicite. Newsletter SMTP réelle et sécurité admin avancée (2FA/CAPTCHA/liste noire IP) de la Phase 4 également non traitées, toujours en attente d'une décision de Jaslin sur la priorité.

## 2026-08-06 (suite)

### SMART GROS-MORNE : Phase 3 (Recherche, filtres réels et carte interactive) livrée

**Contexte :** sur "vas y", suite directe après la Phase 2f livrée plus tôt dans la même journée.

**Constat de départ, avant tout code :** audit du frontend a révélé que Tourisme avait déjà, sans documentation nulle part, une recherche serveur (`q` sur nom/description) **et** une carte Leaflet réelle et générique (`CarteInteractive.tsx`, `leaflet`/`react-leaflet` bien installés) — probablement construit en marge d'une session antérieure non `/update`ée, comme le module `recherche` (recherche globale) déjà repéré en fin de Phase 2f. L'Annuaire avait aussi déjà sa carte + une recherche, mais **entièrement côté client** : `AnnuaireSection.tsx` chargeait les 7 verticaux en entier via `Promise.all` (sans aucun filtre passé au serveur) puis filtrait nom/catégorie en mémoire dans le navigateur — fonctionnel au volume actuel mais ni "réel" ni scalable au sens du cahier des charges ("recherche et filtres réels sur les sections à fort volume"). Seuls Actualités et Agenda n'avaient strictement aucune recherche, ni serveur ni client. Ce constat a considérablement réduit le périmètre réel de la Phase 3 par rapport à ce que `PLAN.md` laissait supposer.

**Livré :** recherche serveur (`q`, `contains`/`insensitive`, mêmes conventions Zod/Prisma que Tourisme) ajoutée à **Actualités** (titre + résumé + contenu) et **Agenda** (nom + lieu + description, sans toucher au filtre "événements à venir" déjà en place). UI de recherche (debounce 300ms) ajoutée aux deux pages dédiées `/actualites` et `/agenda` — composants `ActualitesSection.tsx`/`AgendaSection.tsx` qui ne sont utilisés QUE sur ces pages, jamais sur l'accueil (qui a ses propres composants teaser compacts `ActualitesAgendaSection`), donc aucun risque de faire apparaître une barre de recherche sur la page d'accueil. Bouton "Voir toutes les actualités" retiré au passage : sans `href`, purement décoratif, la page affichait déjà la liste complète en dessous.

**Annuaire converti en recherche server-side réelle** : nouvel endpoint `GET /api/annuaire/toutes?secteur=&q=` agrégeant les 7 verticaux (Business/Hotel/Restaurant/School/HealthFacility/Association/AnnuaireEntry) directement en base via `Promise.all` de requêtes Prisma filtrées — `q` et `secteur` poussés dans chaque `where`, seuls les résultats déjà filtrés transitent sur le réseau (contre 100% des enregistrements auparavant). Les libellés de catégorie (dupliqués jusqu'ici dans le frontend) résolus une seule fois côté serveur, la carte/fiche unifiée renvoyée directement dans la forme attendue par le composant. `AnnuaireSection.tsx` simplifié en conséquence (un seul appel API au lieu de 7 + `useMemo` de filtrage). **Bug latent découvert et documenté en écrivant le backend** (pas un bug qui affectait le rendu, juste une incohérence de typage) : le code client référençait `latitude`/`longitude` sur École et Association pour la carte, deux modèles qui **n'ont pas ces champs en base** — ces fiches n'ont donc jamais pu apparaître sur la carte interactive, la vérification TypeScript stricte du backend l'a immédiatement révélé là où le typage plus lâche du frontend laissait passer silencieusement `undefined`. Corrigé en passant `null` explicitement pour ces deux verticaux plutôt que de référencer un champ inexistant.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend (0 erreur, y compris après la découverte et la correction du bug latitude/longitude ci-dessus). API (curl + scripts Node — recherche Actualités avec résultat et sans résultat, recherche Agenda, agrégation Annuaire par secteur exact `hotels`→1 résultat et par texte `Lycée`→1 résultat, comptage par secteur sur la vue "toutes" : entreprises 2/hôtels 1/restaurants 1/écoles 4/santé 5/associations 5). Navigateur : recherche Actualités tapée en direct ("clinique" → 8 fiches ramenées à 1 résultat pertinent, affichage temps réel confirmé), filtre secteur "Santé" sur l'Annuaire (18 → 5 fiches, requête serveur confirmée), non-régression de la page Tourisme (carte Leaflet réelle et recherche déjà existantes toujours fonctionnelles). **Incident d'outillage rencontré et résolu** : après redémarrage du serveur Next.js pour la vérification, `/tourisme` a répondu 404 malgré un fichier de route strictement intact — même bug Turbopack déjà documenté dans l'historique du projet en Phase 2a/2c (une route existante non détectée par un serveur Turbopack déjà démarré), résolu par un redémarrage avec cache `.next` vidé (`rm -rf .next`).

**Reste ouvert, non traité dans cette phase (documenté plutôt que deviné)** : les catégories `AnnuaireEntry` hors BANQUE (églises, ONG, stations-service, boutiques, garages, professionnels, transport) restent sans filtre secteur dédié — visibles seulement sous "Toutes", même limite héritée du comportement client précédent, non étendue ici faute de demande explicite. Recherche insensible aux accents non implémentée (`contains` Postgres standard sans extension `unaccent` — "marche" ne trouve pas "marché", cohérent avec le comportement déjà en place sur Tourisme depuis la Phase 2b, pas une régression introduite ici). L'agrégateur Annuaire ne pagine pas (18 fiches actuellement, acceptable, à revisiter si le volume grandit significativement). Serveurs arrêtés proprement en fin de session (le port 4010 confirmé libre par un `curl` échouant, même précaution que la Phase 2f suite au risque de PID zombie `nodemon`/`ts-node` sous Windows).

**Ceci clôt la Phase 3 du plan SMART GROS-MORNE.** Reste : Phase 4 (commentaires/modération, notifications internes, envoi réel de newsletter, bibliothèque de fichiers générique, gestion de menus dynamique, 2FA/liste noire IP/CAPTCHA) et Phase 5 (SEO technique, i18n réel FR/HT effectif sur tout le site, performance) — toutes deux non commencées, explicitement hors scope tant que non redemandées par Jaslin.

## 2026-08-06

### SMART GROS-MORNE : Phase 2f (Pages transverses & configuration) livrée — Phase 2 entièrement close

**Contexte :** sur "CONTINUER AVEC GROSMORNE", reprise du phasage à l'endroit exact où il avait été laissé (Phase 2f jamais commencée, `PLAN.md` vérifié comme source de vérité avant de coder). Dernier chantier de la Phase 2 avant la Phase 3 (recherche/filtres serveur, carte interactive).

**Livré :** 6 nouveaux modèles Prisma + 1 middleware générique. `Temoignage`/`TemoignageTraduction` (nom/fonction/photo/note invariants, contenu traduit FR/HT) — nouvelle section `TemoignagesSection` sur l'accueil, entièrement absente du contenu hérité, masquée tant qu'aucun témoignage n'est publié (même principe que les Vidéos en Phase 2d : jamais de contenu fabriqué pour combler un module vide). `Partenaire` (fusion sponsors+partenaires validée avec Jaslin en amont du phasage) avec un enum `emplacements` volontairement restreint à `ACCUEIL`/`A_PROPOS` — les deux seuls endroits du site qui affichaient déjà des partenaires en dur. `MessageContact` (formulaire public `/contact`, aucune authentification requise pour écrire, rate limiting dédié `formulairePublicLimiter` 20/15min) et `AbonneNewsletter` (upsert idempotent — une réinscription après désabonnement réactive plutôt que d'échouer sur la contrainte unique — export CSV, toujours aucun envoi de campagne comme prévu au phasage). `Faq`/`FaqTraduction` avec un enum catégorie reprenant les 3 catégories déjà utilisées par le contenu hérité (Tourisme/Services/Investissements) et les 2 catégories supplémentaires du cahier des charges (Démarches, Fonctionnement du site). `SiteSettings` (singleton `id="singleton"` fixé côté service, jamais côté client) alimentant Footer/TopBar/page Contact.

**Journal d'activité (`ActivityLog`) : un seul middleware plutôt qu'un retrofit de ~20 modules.** Plutôt que d'ajouter un appel de log dans chaque contrôleur existant, un middleware générique unique (`activityLog.middleware.ts`) capture automatiquement toute requête admin mutante (POST/PUT/PATCH/DELETE) réussie via `res.on('finish')`. Dérive l'entité et l'id depuis `req.originalUrl` (jamais `req.baseUrl`, dont la valeur dépend de façon peu fiable de l'empilement des routeurs Express traversés) — fonctionne correctement même si le middleware est monté avant les routes elles-mêmes, puisque `req.user` (posé par `requireAuth`, plus loin dans la chaîne) est déjà peuplé sur le même objet requête au moment où le callback `finish` s'exécute. Effet secondaire non bloquant (une panne d'écriture du journal n'échoue jamais une requête métier déjà répondue). Vérifié en conditions réelles : les actions sur les 6 nouveaux modules **et** sur des actions au hasard hors phase (changement de statut d'un message de contact) sont apparues dans le journal sans aucune configuration par module. Écran admin `/admin/journal-activite` avec filtre dynamique par module et pagination.

**Nettoyage au passage (même défaut, même correctif) :** `TopBar.tsx` avait exactement le même défaut que l'ancien `Footer.tsx` (icônes réseaux sociaux `href="#"`, non fonctionnelles) — corrigé avec le même hook partagé `useSiteSettings` : une icône n'est visible que si son lien est réellement renseigné dans Paramètres, jamais une affordance simulée. Hook `useSiteSettings` créé (`frontend/src/hooks/`, dossier inexistant jusqu'ici) pour éviter de dupliquer le fetch entre Footer/TopBar/ContactSection.

**Migration du contenu hérité réel :** les 6 questions/réponses de l'ancien `FaqSection.tsx` (Tourisme ×2, Services ×2, Investissements ×2) et les 2 vrais partenaires de l'ancien `SponsorsSection.tsx` (Francisque FM 98.9, Mairie de Gros-Morne — distincts des 4 placeholders génériques "Votre commerce ici" etc., qui restent affichés à la suite des vrais partenaires plutôt que d'être remplacés) migrés tels quels, sans texte inventé. Sur `AProposSection.tsx`, en revanche, les 5 noms d'institutions non vérifiées (BRH, USAID, Digicel, Sogebank, Mairie) signalées dans l'audit du 2026-08-02 ont été **retirées** plutôt que migrées — le bloc "Partenaires & soutiens" ne s'affiche désormais que si de vrais partenaires publiés existent pour l'emplacement `A_PROPOS` (vide pour l'instant).

**Incident détecté en vérification (pas en lisant le code) :** un premier essai de migration des FAQ via un script bash (curl avec accents dans les arguments `-d`) a silencieusement corrompu l'encodage UTF-8 des caractères accentués en base (confirmé indépendamment via un script Node, pas un simple artefact d'affichage du terminal). Les 6 entrées corrompues supprimées et recréées via un script Node.js dédié (`http.request` avec `Content-Type: application/json; charset=utf-8` explicite) — vérifié par relecture de l'API, accents corrects.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend (0 erreur). API (curl + script Node — CRUD complet testé sur les 6 nouveaux modules, formulaire de contact et newsletter confirmés accessibles sans authentification, `/api/activity-log` confirmé 401 sans token, entrées correctement journalisées après chaque action admin). Navigateur (accueil avec le vrai témoignage et les 2 vrais partenaires + 4 placeholders, page FAQ avec les 3 catégories réelles, formulaire de contact soumis réellement avec passage automatique en "Lu" une fois ouvert côté admin — visible dans le journal d'activité —, 7 écrans admin ouverts et un cycle d'édition testé sur Partenaires). Données de test nettoyées après vérification ; le journal d'activité lui-même **conservé tel quel** (c'est un journal d'audit réel des actions de cette session, pas un artefact à purger). Serveur backend arrêté proprement en fin de session (les PID `ts-node`/`nodemon` se respawnent sous Windows si on ne tue pas tout l'arbre de processus — confirmé arrêté par `curl` échouant sur le port 4010).

**Ceci clôt entièrement la Phase 2 (2a-2f) du plan SMART GROS-MORNE.** Reste : **Phase 3** (recherche/filtres serveur réels sur Annuaire/Tourisme/Actualités/Agenda, carte interactive Leaflet façon LAKAY) — un module `recherche` (recherche globale légère sur personnalités/actualités/tourisme, `GET /api/recherche?q=`) existe déjà dans le code mais ne couvre pas le périmètre complet de la Phase 3 telle que scopée dans `PLAN.md`, et n'était documenté nulle part avant cette session (probablement construit en marge d'une session antérieure non `/update`ée, même schéma que d'autres découvertes non documentées sur ce portefeuille). Phase 4 (commentaires/modération, notifications internes, envoi réel de newsletter, 2FA...) et Phase 5 (SEO technique, i18n réel, performance) restent non commencées, hors scope tant que non redemandées.

## 2026-08-02 (suite)

### Manuel professionnel d'Administration Système et Infrastructure : les 86 chapitres et les 6 annexes livrés

**Contexte :** commande initiale de Jaslin agissant comme un comité international d'experts sysadmin/infrastructure/sécurité, pour un manuel de formation complet couvrant l'administration système au niveau entreprise (Windows Server, Linux, virtualisation, cloud, conteneurs/Kubernetes, automatisation/IaC, supervision, réseau avancé, cybersécurité/gouvernance), distinct des manuels déjà existants `manuel-reseau-entreprise` et `manuel-windows-terminal`. Consigne explicite de Jaslin après la clarification du périmètre : « depuis le début, quelque chose de très profond qui donne l'impression que ce sont des humains, une version terre à terre compréhensible par un débutant » — chapitre 1 validé comme gabarit pilote (2026-07-23 sur `manuel-nodejs`), puis les 85 chapitres et 6 annexes suivants écrits en continu sur confirmations successives (« continue »/« vas y ») sans jamais revenir sur le périmètre ni la qualité demandée.

**Livré :** 86 chapitres répartis en 13 parties (Métier/ITIL/documentation ; Windows Server avancé ; Linux avancé ; Identité et Zero Trust ; Stockage/PRA/PCA ; Virtualisation ; Conteneurs/Kubernetes ; Cloud AWS/Azure/GCP/FinOps ; Automatisation Git/Ansible/Terraform/Jenkins/DevSecOps ; Supervision Zabbix/Prometheus/Grafana/ELK/Graylog/Wireshark ; Réseau avancé Cisco/Fortinet/Mikrotik/proxy/VPN/VLAN ; Cybersécurité et gouvernance NIST/ISO 27001/CIS/SIEM/IDS-IPS/EDR/pentest/réponse à incident ; Projet final d'infrastructure hybride 300 employés multi-sites), plus 6 annexes (aide-mémoire commandes, glossaire, comparatif certifications, modèles de documents, erreurs fréquentes récapitulées, ressources officielles). Fil rouge unique tenu sur l'intégralité du manuel : une compagnie d'assurance haïtienne fictive, 200 puis 300 employés, sites Port-au-Prince/Cap-Haïtien/bureau satellite/nouveau site, partant d'un incident de rançongiciel fondateur (chapitre 4, accès RDP exposé) jusqu'à la remise d'un projet d'infrastructure hybride complet et gouverné (chapitre 86) — chaque chapitre référence explicitement les décisions et incidents des chapitres précédents plutôt que de traiter chaque sujet en silo.

**Pipeline de build établi et fiabilisé** : `build.ps1` (calqué sur les autres manuels du portefeuille) assemble les chapitres Markdown, pré-rend les diagrammes Mermaid en SVG, puis génère HTML/DOCX/PDF via pandoc + Puppeteer. Deux bugs réels trouvés et corrigés sur le chapitre pilote : (1) `--embed-resources` de pandoc échouait sur des chemins absolus Windows avec lettre de lecteur (interprétés comme un schéma d'URI) — corrigé en passant des chemins relatifs après `Set-Location` vers la racine du projet ; (2) un bug de splat PowerShell sur un tableau à un seul élément (`ForEach-Object` renvoie une chaîne, pas un tableau, sur 1 item) faisait exploser chaque caractère du nom de fichier en argument séparé pour pandoc — corrigé en forçant `@(...)`. Un troisième problème, purement transitoire (verrou de profil Puppeteer/Edge en cas de builds lancés en succession rapprochée), rencontré à plusieurs reprises sans lien avec le contenu — toujours résolu par une simple relance du build suivant.

**Discipline de vérification tenue sur les 92 fichiers** : après chaque chapitre, comptage systématique des balises `<div>`/`</div>` (un bug récurrent d'auto-introduction — une boîte "Questions ouvertes" laissée non fermée juste avant `## Exercices` — détecté et corrigé des dizaines de fois via ce contrôle) et vérification qu'aucune classe CSS invalide (`en-entreprise`, qui doit toujours rester du texte brut avec un lead-in en gras, jamais un encadré) ne s'était glissée. Chaque chapitre suivi d'un rebuild en arrière-plan, confirmé par la présence des trois exports (HTML/DOCX/PDF) sans avertissement avant de passer au suivant.

**Résultat final vérifié** : build complet propre, 92 fichiers assemblés (86 chapitres + 6 annexes), HTML/DOCX/PDF générés sans erreur ni avertissement. Livrables dans `livrables/formations/manuel-administration-systeme/export/`.

**Prochaine étape** : aucune côté manuel — travail complet et livré. Reste ouvert si Jaslin souhaite une relecture ciblée, un export supplémentaire, ou une intégration avec les deux autres manuels du portefeuille (`manuel-reseau-entreprise`, `manuel-windows-terminal`).

## 2026-08-02

### SMART GROS-MORNE : fiches de détail (personnalités/articles) + refonte de l'accueil et de la page Histoire (hors-phasage, sur maquettes fournies par Jaslin)

**Contexte :** Jaslin a fourni 4 images de référence (logo de la commune, maquette de la page Personnalités liste + fiche détail, maquette de la page Histoire, maquette des sections restantes de l'accueil) avec la consigne : aligner les sections manquantes de l'accueil et la disposition des autres pages sur ces maquettes, et faire en sorte qu'un clic sur un article/une actualité mène vers une vraie page de détail (jusqu'ici inexistante).

**Backend — modèle `Personnalite` enrichi (migration additive) :** nouvel enum `CategorieHonneur` (Politique & Gouvernance / Culture & Arts / Éducation & Sciences / Sports / Entrepreneuriat / Autres) sur le parent, plus `dateNaissance`/`lieuNaissance`/`nationalite`/`profession`/`periodeActivite` (invariants par langue) ; `realisations[]` et `citation` ajoutés à la traduction (nécessaires pour la fiche détail de la maquette : informations clés, réalisations majeures, citation). Nouveaux endpoints publics unitaires `GET /personnalites/:id` et `GET /articles/:id` (ce dernier 404 si l'article n'est pas `PUBLIE`, même garde que le reste du site) — jusqu'ici seules des listes existaient, aucune fiche de détail n'était consultable individuellement. **Bug réel trouvé et corrigé en vérification** : le service d'upsert de traduction passait `citation: t.citation` tel quel, et Prisma ignore silencieusement une clé `undefined` dans un `update` — une fois une citation renseignée depuis l'admin, il devenait impossible de la supprimer en vidant le champ du formulaire. Corrigé en normalisant explicitement en `null` (`t.citation || null`), vérifié en reproduisant le scénario exact (poser une citation, la vider, confirmer `citation: null` en base).

**Frontend — nouvelles pages de détail :** `/personnalites/[id]` (photo, nom, domaine, badge de catégorie, onglets Biographie/Parcours/Réalisations/Galerie/Actualités — seuls Biographie et Réalisations sont fonctionnels, les trois autres affichent honnêtement « disponible prochainement » faute de modèle de données pour un parcours détaillé, une galerie par personnalité ou un lien vers les actualités qui la mentionnent ; sidebar Informations clés + Réalisations majeures + citation, chacun masqué si vide) et `/actualites/[id]` (image, résumé, contenu, tags, partage). Nouveau composant `Breadcrumb.tsx` partagé par les deux. La liste `/personnalites` a reçu des tabs de filtre par catégorie + une recherche par nom ; `ActualitesSection` et la nouvelle section d'accueil pointent désormais réellement vers ces fiches (avant : boutons « Lire » non cliquables, cartes de l'accueil pointant vers des pages statiques sans rapport avec le contenu réel).

**Frontend — accueil reconstruit pour matcher la maquette :** les anciennes sections (`FeaturedSection`, `HomeOverview`, `GalleryPreview`, `SectionsCommunes`, `DonationSection`, `TickerBanner`) dataient d'avant la refonte visuelle de la Phase 1 et détonnaient avec elle (magazine sombre, dons fictifs en dollars avec barres de progression inventées) — supprimées et remplacées par : `DecouvrirSection` (4 cartes), `StatsBarSection` (« Gros-Morne en chiffres », avec **comptages réellement interrogés en direct** sur les API existantes — sections communales, écoles, entreprises, sites touristiques — plutôt que des chiffres statiques comme le fait déjà le Hero), `LieuxIncontournablesSection` (carrousel des lieux touristiques réels), `ActualitesAgendaSection` (3 dernières actualités + prochains événements, état honnête « aucun événement publié » si l'agenda est vide), `InvestirBanner` (bandeau, renvoie vers `/investir`), `SponsorsSection` restylée en bandeau de logos tout en **conservant le principe des emplacements génériques déjà en place** (« Votre commerce ici », etc.) plutôt que d'adopter les noms d'institutions réelles visibles sur la maquette (BRH, USAID, Digicel, Sogebank, Mairie, ONU) — aucune preuve de partenariat réel avec ces organismes, décision documentée plutôt que silencieuse. Note laissée ouverte : la maquette a une note de fiabilité par lieu touristique (ex. « 4.8 (120) ») qui n'a pas été reprise, aucune donnée d'avis n'existant (même principe que le retrait des faux « likes » de la Galerie en Phase 2d).

**Frontend — page Histoire refaite :** l'ancienne section (frise verticale en zigzag) remplacée par la disposition de la maquette : sommaire latéral avec ancres, aperçu historique, emplacement vidéo honnêtement marqué « à venir » (aucune vidéo réelle n'existe pour cette page), cartes des « grandes périodes », frise horizontale des faits marquants, bandeau final. Contenu conservé identique à l'existant (5 périodes déjà utilisées ailleurs) plutôt que d'adopter les dates très précises mais non vérifiables de la maquette (1690, 1730, 1791, 1864...).

**Navbar/Footer alignés sur la maquette :** bouton « Rejoindre » remplacé par « Espace admin » (lien `/admin/login`, cohérent avec les 4 captures fournies) ; `/communaute` reste accessible via le méga-menu Découvrir pour ne pas devenir une page orpheline. Footer passé de 4 à 5 colonnes (ajout d'« Informations utiles » : à propos/FAQ/mentions légales/confidentialité/CGU).

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend. Navigateur : accueil (tous les nouveaux blocs rendus avec données réelles, compteurs live corrects), liste et fiche Personnalités (mise à jour réelle via l'API avec tous les nouveaux champs, y compris réalisations et citation, puis nettoyage des données de test), fiche Article, page Histoire — toutes 200, sans erreur console. Un incident sans rapport avec le code (nouvelle route `[id]` non détectée par le serveur Turbopack déjà démarré, 404 jusqu'au redémarrage avec cache `.next` vidé) rencontré et documenté, cohérent avec les incidents Turbopack déjà notés sur ce projet. Base remise à son état d'origine après vérification.

**Reste ouvert, signalé à Jaslin plutôt que deviné :** (1) le logo réel de la commune (image fournie en pièce jointe) n'a pas pu être extrait en fichier utilisable depuis le chat — le Navbar/Footer utilisent toujours un logo générique (cercle + icône), à remplacer si Jaslin fournit le fichier ; (2) incohérence préexistante non introduite par cette session : la population de Gros-Morne apparaît sous 3 valeurs différentes selon les pages (132 245 dans le Hero/accueil, ~155 692 déductible de la densité × superficie affichées sur `/geographie`) — jamais réconciliée, à trancher par Jaslin ; (3) Phase 3 du plan initial (recherche/filtres serveur réels, carte interactive) reste non commencée, ce chantier d'aujourd'hui portait sur l'alignement visuel et les fiches de détail, pas sur ce périmètre.

## 2026-07-29 (suite 2)

### SMART GROS-MORNE : correctif images de fond des héros (hors-phasage)

**Contexte :** retour direct de Jaslin après la Phase 2e : "LA PAGE D'ACCUEIL N'EST PAS COMME JE L'AI VOULAIS. CHAQUE HERO DOIT AVOIR UNE IMAGE DE FON ETC." Deux clarifications obtenues : (1) les images doivent être gérables depuis l'admin (pas codées en dur), (2) Jaslin uploade lui-même ses vraies photos — pas de photo de stock de remplacement.

**Diagnostic :** le hero de l'accueil (`HeroSection.tsx`) faisait tourner un faux carrousel de 3 slides identiques, toutes la même photo Wikipedia d'une cascade de **Ville Bonheur** (une autre commune) — factuellement fausse pour Gros-Morne. Les 23 autres pages n'avaient soit aucune image de fond, soit (pour `/tourisme` et un bloc décoratif d'`InvestirSection.tsx`) la même photo Wikipedia incorrecte en dur.

**Livré :** modèle `PageHeroImage` (clé `page` parmi 24 valeurs, relation vers `Media` déjà existant — aucun stockage dédié). Module backend `hero-images` (GET public par page, GET/PUT/DELETE admin ; route `/admin` déclarée avant `/:page` pour ne pas se faire intercepter par le paramètre enum). Composant `DynamicPageHeader` câblé sur les 23 pages secondaires (fetch + repli sur le gradient déjà supporté par `PageHeader`) et réécriture complète de `HeroSection.tsx` (accueil) pour retirer le faux carrousel. Écran admin `/admin/images-hero` (grille des 24 pages, upload direct ou réutilisation d'un média déjà en bibliothèque, suppression), lien ajouté en 2e position dans la sidebar admin. Photo Wikipedia incorrecte retirée de ses deux emplacements (hero Tourisme, bloc décoratif Investir) et remplacée par un gradient neutre — aucune photo de substitution insérée, conformément à la décision de Jaslin.

**Vérifié en conditions réelles** (et non simulées) : pendant la session de vérification, Jaslin a lui-même uploadé 3 vraies photos via l'admin fraîchement construit (accueil, histoire, géographie) — confirmé via `curl` sur `/api/hero-images/admin` puis, page par page dans le navigateur, chaque page affiche bien sa propre image uploadée (3 URLs de fichiers distinctes) ; `/culture` (page sans upload) confirmée en repli gradient neutre, sans image cassée ni contenu de substitution. `tsc --noEmit` propre backend/frontend.

**Bug réel trouvé et corrigé immédiatement après (remonté par Jaslin : "les images s'affichent pas que côté admin ni dans les héros")** : les URLs de médias sont stockées en base sous forme relative (`/uploads/xxx.webp`, servies par `express.static` sur le backend port 4010) mais chaque endroit du frontend qui affichait une image l'utilisait telle quelle comme `src`, donc la requête partait vers l'origine du frontend (port 3010) au lieu du backend — 404 systématique. Bug préexistant et généralisé, jamais remarqué avant car c'était la première fois qu'une vraie photo uploadée était affichée sur le site public (aucune Phase précédente n'avait de photo réelle en base). Touchait en réalité 14 fichiers : les 6 nouveaux (hero images + son admin) mais aussi 8 déjà existants — les 9 sélecteurs de médias admin (Tourisme, Actualités, Entreprises, Hôtels, Restaurants, Écoles, Santé, Associations, Annuaire), la Galerie publique (`GalerieSection.tsx`/`GalleryPreview.tsx`) et les Personnalités (`PersonnalitesSection.tsx`). Corrigé avec un helper unique `mediaUrl()` dans `lib/api.ts` (préfixe l'URL relative avec l'origine du backend déduite de `NEXT_PUBLIC_API_URL`), appliqué à chaque point d'affichage plutôt que de changer le format de stockage en base. Vérifié en navigateur (network + rendu visuel réel, plus de 404) sur l'accueil et l'admin `/admin/images-hero` avec les vraies photos que Jaslin continuait d'uploader pendant la correction. `tsc --noEmit` propre.

**Prochaine étape :** aucune (correctif ponctuel, pas une phase du roadmap). Phase 3 (recherche/filtres serveur, carte interactive) reste en attente d'une confirmation explicite de Jaslin avant de démarrer.

## 2026-07-29 (suite)

### SMART GROS-MORNE : Phase 2e (Répertoires métier) livrée — plus gros chantier de la Phase 2

**Contexte :** sur "VAS Y" puis "CONTINNUE" après une interruption technique, suite directe du phasage réactualisé. Chantier le plus large de la Phase 2 : 8 nouveaux modèles Prisma, 8 modules backend, 8 écrans admin, câblage de 6 pages publiques.

**Livré :** `Business` (Entreprises), `Hotel`, `Restaurant`, `School` (Éducation), `HealthFacility` (Santé), `Association` (Vie associative), `MunicipalService` (Services municipaux — seul modèle sans photo, décision assumée pour ne pas sur-construire un module qui n'en avait pas besoin), `AnnuaireEntry` (catch-all pour les catégories sans module dédié : banques/églises/ONG/stations-service/boutiques/garages/professionnels/transport). Chacun avec sa table de traduction FR/HT, suivant scrupuleusement le même schéma que les phases précédentes. Client API frontend factorisé (`repertoireApi(base)`) puisque les 8 modules partagent exactement la même forme REST — la seule simplification de ce type dans toute la Phase 2, les 8 écrans admin eux-mêmes restant des fichiers distincts (leurs champs diffèrent réellement).

**Décision de reconciliation majeure** : `/annuaire` (page publique) agrège désormais 7 des 8 modèles via `Promise.all` côté client — `MunicipalService` explicitement exclu, absent de la liste de catégories de l'Annuaire dans le cahier des charges. Le filtre/recherche client déjà existant (7 onglets : Toutes/Entreprises/Écoles/Santé/Hôtels/Restaurants/Banques/Associations) a été conservé tel quel plutôt que d'ajouter immédiatement des onglets pour les catégories `AnnuaireEntry` encore vides (églises, ONG...) — un vrai filtre serveur multi-catégories reste explicitement le travail de la Phase 3.

**Décision de contenu assumée sur Économie et Investir** : ces deux pages du cahier des charges n'ont **pas** été câblées sur un modèle de répertoire. Leur contenu hérité (`economie/page.tsx`, `InvestirSection.tsx`) est une prose descriptive de secteurs économiques abstraits (Agriculture, Commerce, Industrie...), pas une liste d'entités nommées comme les 6 autres pages — les vraies entreprises nommées du site (ETS Construction Bâtiment, Super Marché Gros-Morne) vivaient uniquement dans l'ancien Annuaire statique, pas sur la page Économie. Câbler ces deux pages sur un modèle de répertoire aurait dénaturé leur contenu réel ; elles restent statiques, décision documentée plutôt que silencieuse.

**Déduplication réelle effectuée** : l'ancien `AnnuaireSection.tsx` (statique) listait 8 fiches dont 3 étaient déjà présentes ailleurs dans le site sous une forme plus riche — Lycée National de Gros-Morne (déjà sur `/education`), Hôpital Saint-Antoine (déjà sur `/sante`), Association des Femmes Unies (déjà sur `/vie-associative`). Chacune migrée **une seule fois** vers son modèle dédié (avec les champs les plus complets des deux sources fusionnés, ex. l'adresse "Section Pignon" de l'Annuaire ajoutée à l'association déjà décrite sur sa propre page), jamais dupliquée entre deux tables.

**Contenu jamais documenté ailleurs, migré avec une description inventée mais non-factuelle** : 2 entreprises (ETS Construction Bâtiment, Super Marché Gros-Morne), 1 hôtel (Hôtel La Montagne) et 1 restaurant (Restaurant Ti Bèl Kote) n'existaient que dans l'ancien Annuaire statique, qui ne portait ni description ni catégorie détaillée pour aucune fiche — contrairement aux pages Éducation/Santé/Vie associative/Services municipaux qui avaient toutes une vraie description d'origine. Une description générique d'une phrase a été rédigée pour chacune (reformulant simplement ce que le nom et la catégorie impliquent déjà, ex. "Entreprise locale de construction et de bâtiment...") — décision distincte de celle prise sur Tourisme en Phase 2b où aucun nouveau lieu n'avait été inventé : ici les entités elles-mêmes sont réelles et déjà publiques, seule une phrase de présentation minimale et non-factuelle manquait pour satisfaire le champ obligatoire.

**`AnnuaireEntry` reste vide** : aucune vraie fiche banque/église/ONG/station-service/boutique/garage/professionnel/transport n'existait dans le contenu hérité (la catégorie "Banques" de l'ancien Annuaire n'avait déjà aucune fiche réelle) — cohérent avec la décision prise sur les Vidéos en Phase 2d de ne jamais fabriquer de contenu pour combler un module structurellement vide.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend/frontend (après chaque lot de modules) ; API (curl — 401 sans token sur les 8 endpoints, comptes exacts vérifiés un par un : entreprises 2, hôtels 1, restaurants 1, écoles 4, santé 5, associations 5, services municipaux 7, annuaire 0) ; navigateur — page `/annuaire` confirmée avec 18 fiches agrégées et filtre "Écoles" ramenant exactement 4 résultats, les 5 pages `FicheCardGrid` (Éducation/Santé/Vie associative/Services municipaux + Annuaire) confirmées avec un contenu identique à l'ancien contenu statique, cycle d'édition réel confirmé sur l'écran admin Entreprises (traduction Kreyòl ajoutée et persistée). Serveurs arrêtés, processus Node orphelins nettoyés explicitement (même limitation Windows récurrente sur ce portefeuille).

**Prochaine étape** : Phase 3 (recherche/filtres serveur réels, carte interactive) — dernière phase de câblage prévue avant les fonctions avancées différées (Phase 4) et le SEO/i18n technique (Phase 5). L'essentiel du cahier des charges enrichi (structure des 20 sections + contenu dynamique) est désormais couvert.

## 2026-07-29

### SMART GROS-MORNE : Phase 2d (Galerie & Vidéos) livrée

**Contexte :** sur "vas y" après la Phase 2c, suite directe du phasage réactualisé.

**Livré :** modèles Prisma `GalerieAlbum`/`GalerieAlbumTraduction` (nom invariant, description traduite FR/HT) et `GalerieMedia` (média individuel d'un album : titre/auteur/lieu, `mediaId` nullable + `icone` emoji en repli). `Video`/`VideoTraduction` en module séparé (lien externe YouTube/Vimeo, `miseEnAvant`, jamais d'upload vidéo lourd — cohérent avec la contrainte data mobile du reste du portefeuille), conformément au découpage du cahier des charges qui traite la Galerie (module 8) et les Vidéos (module 9) comme deux modules admin distincts. Nouvel enum `CategorieGalerie` (9 valeurs, réconciliant les catégories du cahier des charges avec celles déjà utilisées dans le code hérité : Nature/Culture/Vie locale/Architecture/Événements). Admin `/admin/galerie` (formulaire imbriqué : champs d'album + liste dynamique de médias avec ajout/suppression de lignes) et `/admin/videos`.

**Décision de contenu assumée** : le contenu hérité de `GalerieSection.tsx` n'a jamais eu de vraies photos — chaque "photo" est en réalité un emoji de substitution (🏔️, 🛒, 🎉...) sur un fond dégradé, jamais une image réelle. Plutôt que d'ignorer cette réalité, le schéma reflète honnêtement cet état : `GalerieMedia.mediaId` est nullable avec `icone` comme repli explicite, et les 8 "photos" existantes ont été migrées telles quelles (mêmes emoji/titre/auteur/lieu) réparties dans **5 albums thématiques créés pour l'occasion** (Nature, Vie locale, Événements, Architecture, Culture — un par catégorie réellement utilisée dans le code d'origine, la structure "album" elle-même n'existait pas avant). Le compteur de "likes" fictif (ex: "42 likes") du code hérité a été retiré plutôt que migré — aucune donnée d'engagement réelle n'existe, l'afficher aurait présenté une métrique fabriquée comme réelle. Aucune vidéo n'a été migrée ou inventée : la fonctionnalité Vidéos est entièrement nouvelle (zéro précédent dans le code hérité), la base reste donc vide jusqu'à ce que Jaslin ajoute de vrais liens.

**Extension hors du périmètre strict de la phase, jugée nécessaire** : `GalleryPreview.tsx`, l'aperçu de la galerie sur la page d'Accueil, dupliquait les mêmes 6 "photos" avec les mêmes faux likes dans un fichier séparé. Laisser l'accueil afficher des données fictives différentes de la vraie page Galerie nouvellement dynamique aurait été incohérent — câblé sur la même API `galerieApi.list()`, mêmes principes (emoji de repli, pas de compteur fictif).

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend/frontend ; API (curl — 401 sans token, filtre catégorie, validation Zod nom manquant, cycle création/lecture/suppression d'une vidéo de test) ; navigateur — **incident d'outillage sévère cette fois** (`get_page_text`/`screenshot`/`javascript_tool`/`navigate` ont échoué en timeout de façon quasi continue sur une portion de la vérification, pane bloqué en état "hidden") contourné en s'appuyant sur `read_page` (qui a continué de fonctionner de façon fiable, contrairement aux phases précédentes où c'était `javascript_tool` le recours) pour confirmer texte et structure, puis `javascript_tool` a fini par redevenir disponible pour les tests d'écriture (édition réelle de l'album Nature avec ajout d'une traduction Kreyòl, persistée et vérifiée ; création réelle d'une vidéo via le formulaire, apparue sur la page publique dans la nouvelle section Vidéos, puis supprimée après vérification pour ne pas laisper de contenu vidéo fictif). Page Galerie publique confirmée affichant les 8 vraies photos réparties par catégorie avec filtres fonctionnels ; aperçu de l'accueil confirmé affichant les 6 premières. **Même précaution que les phases précédentes** : ne jamais conclure d'un échec de rendu sans revérifier par une méthode indépendante (ici `read_page`), un problème d'outillage n'étant pas un bug produit tant qu'il n'est pas confirmé par plusieurs angles. Serveurs arrêtés, processus Node orphelins nettoyés explicitement (même limitation Windows récurrente).

**Prochaine étape** : Phase 2e (Répertoires métier : Entreprises/Hôtels/Restaurants/Écoles/Santé/Associations/Services municipaux/Annuaire générique), pas encore démarrée — le plus gros chantier restant de la Phase 2, 8 nouveaux modèles Prisma.

## 2026-07-28 (suite, 6)

### SMART GROS-MORNE : Phase 2c (Actualités & Agenda) livrée

**Contexte :** sur "continue" après la Phase 2b, suite directe du phasage réactualisé.

**Livré :** modèles Prisma `Article`/`ArticleTraduction` (titre/auteur/tags invariants sur le parent, résumé+contenu traduits FR/HT, image principale en relation simple vers `Media`) et `Evenement`/`EvenementTraduction` (nom/lieu/GPS invariants, description traduite, `heureAffichage` en texte libre plutôt qu'une contrainte d'heure stricte — fidèle aux données réelles du type "Toute la journée"). Deux nouveaux enums `CategorieArticle` (12 valeurs) et `CategorieEvenement` (11 valeurs), calés sur le cahier des charges et étendus pour couvrir le contenu réel existant (même réconciliation que Tourisme en Phase 2b). L'agenda public applique un double filtre : `statutPublication: PUBLIE` **et** `date >= aujourd'hui` — un événement publié dont la date est passée reste invisible côté public (testé explicitement). Écrans admin `/admin/actualites` et `/admin/agenda`, tous deux avec sélecteur de statut (Brouillon/Publié/Archivé). `ActualitesSection.tsx` (featured + side + grille) et `AgendaSection.tsx` (mini-calendrier + liste) câblés sur l'API.

**Bug réel trouvé et corrigé en vérification (pas en lisant le code)** : les dates de publication/événement s'affichaient avec un décalage d'un jour côté client (15 juin 2025 → "14 juin 2025" à l'écran). Cause : les dates sont stockées en UTC minuit (`z.coerce.date()` sur un `"2025-06-15"`), mais `formaterDate()` utilisait `toLocaleDateString("fr-FR", ...)` et le mini-calendrier `getDate()`/`getMonth()` — toutes des méthodes qui appliquent le fuseau **local** du navigateur, décalant d'un jour en arrière sous un fuseau négatif (America/Port-au-Prince, UTC-4, comme le reste du portefeuille). Corrigé en forçant `timeZone: "UTC"` dans `toLocaleDateString` et en remplaçant `getDate()`/`getMonth()`/`getFullYear()` par leurs équivalents `getUTC*` partout où une date d'événement est affichée. Repéré uniquement parce que les dates réelles ont été migrées puis relues à l'écran (`document.body.innerText` via JavaScript, cf. incident d'outillage ci-dessous) — un test avec des dates fictives n'aurait rien révélé si elles tombaient un jour où le décalage ne change pas le mois affiché.

**Décision de contenu assumée sur l'Agenda** : les 6 articles d'Actualités ont été migrés avec leurs dates historiques exactes (mai-juin 2025) et publiés directement — une actualité garde légitimement une date passée. Les 5 événements de l'Agenda, en revanche, ont été migrés en statut **BROUILLON** plutôt que publiés : leurs dates dans le code hérité (25 mai, 10/18/21 juin, 5 juillet) n'indiquaient aucune année, et le mini-calendrier affichait "Mai 2026" en dur sans rapport avec le contenu réel — signe que ces données étaient un gabarit de démonstration, pas un vrai calendrier vérifié. Plutôt que d'inventer une année plausible et publier des événements avec une date potentiellement fausse sur le vrai site, ils restent en brouillon (visibles seulement en admin) jusqu'à ce que Jaslin fournisse les vraies dates.

**Incident d'outillage rencontré et contourné (pas un bug produit)** : le pane de prévisualisation est resté bloqué en état "hidden" une bonne partie de cette vérification — `get_page_text` et `computer{screenshot}` échouaient systématiquement en timeout, alors que `read_page`/`read_network_requests`/`javascript_tool` continuaient de fonctionner normalement. Contourné en lisant `document.body.innerText` directement via JavaScript à chaque étape de vérification (accueil, page Actualités, page Agenda, écrans admin) plutôt que les captures visuelles — a permis de mener la vérification à son terme, y compris la découverte du bug de décalage de date ci-dessus.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend/frontend (avant et après le correctif de date) ; API (curl — RBAC, filtre catégorie, un événement publié à date passée testé explicitement invisible en public, brouillon invisible en public mais visible en admin) ; navigateur (page Actualités publique avec les 6 vrais articles et dates exactes après correctif, page Agenda publique vide tant qu'aucun événement n'est publié — comportement voulu —, un événement de test à date réellement future créé puis publié pour confirmer le rendu complet du calendrier et de la liste, puis supprimé ; écrans admin chargeant les vraies données ; cycle d'édition réel confirmé — traduction Kreyòl ajoutée à un article via le formulaire et persistée). Serveurs arrêtés, processus Node orphelins nettoyés explicitement (même limitation Windows que les phases précédentes).

**Prochaine étape** : Phase 2d (Galerie & Vidéos), pas encore démarrée.

## 2026-07-28 (suite, 5)

### SMART GROS-MORNE : Phase 2b (Tourisme) livrée

**Contexte :** sur "continue" après la Phase 2a, suite directe du phasage réactualisé (entrée précédente).

**Livré :** modèles Prisma `TourismPlace`/`TourismPlaceTraduction` (nom propre invariant sur le parent, description/conseils traduits FR/HT) et `TourismPlacePhoto` (table de jonction vers le modèle `Media` déjà existant depuis la Phase 0, réutilise l'upload générique plutôt que réinventer un système de photos). Deux nouveaux enums : `CategorieTourisme` (13 valeurs, calées sur le cahier des charges — cascades/rivières/montagnes/grottes/églises/sites historiques/sentiers/hébergements/restaurants — étendues à `CULTURE`/`EVENEMENT`/`AUTRE` pour couvrir fidèlement le contenu réel déjà en ligne) et `StatutPublication` (`BROUILLON`/`PUBLIE`/`ARCHIVE`, pensé pour être réutilisé tel quel par Actualités/Événements en Phase 2c). Deux endpoints de lecture distincts : `GET /tourisme` (public, ne renvoie jamais un brouillon/archivé) et `GET /tourisme/admin` (protégé, voit tout). Écran admin `/admin/tourisme` avec un vrai sélecteur de photos (upload direct ou réutilisation d'une image déjà présente dans la bibliothèque `Media`, premier écran du portefeuille gros-morne à exploiter ce module au-delà de son propre CRUD). `TourismeSection.tsx` (page `/tourisme`) câblée sur l'API — icône et couleur de carte dérivées de la catégorie côté client (dictionnaire fixe), jamais stockées en base, même principe que les initiales calculées de `PersonnalitesSection` en Phase 2a.

**Décision de contenu assumée** : les deux captures d'écran fournies par Jaslin avec le cahier des charges enrichi montraient des noms de lieux touristiques très spécifiques (ex. une cascade, une grotte nommée). N'ayant aucun moyen de vérifier l'exactitude de ces noms pour la vraie commune de Gros-Morne, ils n'ont pas été utilisés comme contenu réel — les 6 fiches déjà en ligne dans `TourismeSection.tsx` (Mornes Panoramiques, Sources & Rivières, Forêts, Centre-Ville Historique, Marchés Locaux, Fête Patronale & Rara) ont été migrées telles quelles vers la base de données, sans invention de nouveaux lieux.

**Incident d'environnement rencontré** (pas un bug applicatif) : après la migration Prisma, `prisma generate` a échoué en `EPERM` (fichier `query_engine-windows.dll.node` verrouillé) — un processus `nodemon`/`ts-node` de la session précédente avait survécu à `TaskStop` (même limitation Windows déjà documentée sur d'autres projets du portefeuille). Résolu en identifiant puis en tuant explicitement les processus Node orphelins (`Get-CimInstance Win32_Process` filtré sur la ligne de commande, `Stop-Process -Force`) avant de relancer `prisma generate` avec succès.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend/frontend ; API (curl — 401 sans token, filtre `?categorie=NATURE` correct, catégorie invalide rejetée en 422, validation Zod nom manquant, un brouillon de test invisible en public mais visible en admin puis supprimé) ; navigateur (page `/tourisme` publique affichant les 6 vraies fiches avec icônes/couleurs par catégorie, écran admin chargeant les 6 fiches avec statut "Publié", cycle d'édition réel confirmé — GPS + traduction Kreyòl ajoutés via le formulaire et persistés, vérifiés à la fois dans le tableau admin et via l'API). **Même limitation d'outillage qu'en Phase 2a** (clics de souris synthétiques peu fiables dans le pane de prévisualisation) contournée par un vrai `.click()` DOM déclenché via JavaScript — diagnostic déjà établi la session précédente comme un problème d'outil, pas de code. Serveurs arrêtés (processus Node orphelins de nouveau nettoyés explicitement par précaution). Les 6 fiches réelles restent en base, pas des données de test à purger.

**Prochaine étape** : Phase 2c (Actualités & Agenda), pas encore démarrée.

## 2026-07-28 (suite, 4)

### SMART GROS-MORNE : cahier des charges enrichi, audit + re-phasage, puis Phase 2a livrée

**Contexte :** Jaslin a fourni une version très détaillée du cahier des charges (structure complète des 20 sections, 28 modules de tableau de bord admin, ~35 tables de base de données proposées) accompagnée de deux captures d'écran du site déjà rendu. Passage par EnterPlanMode avant tout code vu l'ampleur.

**Audit réalisé avant de planifier** : malgré la refonte visuelle et les 20 pages livrées en Phase 1 (entrée précédente du jour), le backend réel ne comptait que 3 modules (`auth`, `contenu`, `media`) et aucune des ~30 entités métier proposées n'existait — les 24 pages publiques sont toutes statiques (`grep fetch` = 0 résultat), seul le mécanisme `PageSection` générique est branché à l'admin. **Découverte au passage, non documentée jusqu'ici** : `PLAN.md` local du projet contenait en fait déjà une Phase 1 "élargie" livrée le jour même (refonte header/hero/footer + 14 pages restantes), jamais reflétée dans ce fichier `HISTORY.md` central — corrigé au passage.

**Trois décisions validées avec Jaslin (AskUserQuestion)** avant de re-phaser : (1) rôles en **enum Prisma fixe** plutôt qu'un moteur RBAC générique dynamique (cohérent avec BANKA/OTELA/GESCOM) ; (2) réconciliation des doublons du document brut (Annuaire section #6 relisait des catégories déjà couvertes par des modules dédiés) — un modèle dédié par vertical ayant son propre module admin explicite, un `AnnuaireEntry` générique catch-all pour le reste ; sponsors et partenaires fusionnés en un seul modèle `Partenaire` catégorisé, exactement la recommandation que Jaslin avait lui-même formulée ; (3) audit d'abord, code ensuite. Phasage réactualisé écrit dans `livrables/applications/gros-morne/PLAN.md` (Phases 2a à 2f, puis 3/4/5 inchangées) — détail complet dans ce fichier.

**Phase 2a livrée le jour même, sur choix explicite de Jaslin** (recommandée parmi 4 options) : modèles Prisma `SectionCommunale`/`SectionCommunaleTraduction` et `Personnalite`/`PersonnaliteTraduction` (nom propre invariant sur le parent, description/domaine/biographie traduits FR/HT sur la table fille, même principe que `PageSectionTraduction`). Deux nouveaux modules backend (`sections-communales/`, `personnalites/`) suivant exactement le pattern déjà posé par `modules/contenu/`. Deux écrans admin CRUD (`/admin/sections-communales`, `/admin/personnalites`), réutilisant tel quel le design system existant (Badge/Modal/PageToolbar/EmptyState), liens ajoutés dans `AdminSidebar.tsx`. Câblage réel : `SectionsCommunes.tsx` (grille des 8 sections sur l'accueil) et `PersonnalitesSection.tsx` (page `/personnalites`) passés du tableau en dur à un `fetch` vers l'API — plus aucune donnée statique sur ces deux composants. `GeographieSection.tsx` (prose générale sur le relief/climat, page `/geographie`) volontairement non touché, hors périmètre de cette phase précise.

**Incident réel rencontré et corrigé en vérifiant** : les premières insertions de contenu réel (accents français/kreyòl) via des commandes shell corrompaient l'encodage UTF-8 (`Rivière` devenait `Rivi�re`) — confirmé être un problème d'encodage du shell Bash utilisé pour la vérification, pas un bug de l'application (le message serveur `"Succès"` s'affichait correctement, preuve que Node/Postgres géraient bien l'UTF-8). Corrigé en écrivant les payloads dans des fichiers `.jsonl` via l'outil d'édition (UTF-8 garanti) et en les envoyant à curl via `--data-binary @fichier` plutôt qu'en ligne de commande — toutes les données corrompues supprimées et ré-insérées proprement avant toute vérification.

**Vérifié en conditions réelles** : `tsc --noEmit` propre backend et frontend ; API (curl — 401 sans token, 400 validation Zod nom manquant, CRUD complet) ; navigateur (accueil affichant les 8 vraies sections avec badge "Section principale" et accents corrects, `/personnalites` affichant les 4 fiches avec initiales calculées dynamiquement, les deux écrans admin chargeant les vraies données). **Incident d'outillage rencontré pendant la vérification navigateur** (pas un bug applicatif) : les clics de souris synthétiques du pane de prévisualisation n'atteignaient pas fiablement les boutons (déclenchaient parfois la mauvaise action, ou fermaient un modal déjà ouvert en cliquant son arrière-plan) — diagnostiqué en déclenchant un vrai `.click()` DOM via JavaScript, qui a confirmé que le code applicatif fonctionne correctement (modal d'édition pré-rempli avec les vraies données, traduction Kreyòl ajoutée et persistée avec succès, badge passé de "Manquant" à "Renseigné"). Les 8 sections communales et les 4 personnalités réelles (contenu identique à l'ancien code en dur) sont désormais en base de données, pas des données de test à nettoyer. Serveurs arrêtés après vérification.

**Prochaine étape** : Phase 2b (Tourisme), pas encore démarrée.

## 2026-07-28

### SMART GROS-MORNE : Phase 1 élargie livrée (refonte visuelle + 20 sections)

**Contexte :** juste après la Phase 0, Jaslin a fourni deux maquettes de référence (captures d'écran d'un style civique/institutionnel — top-bar, méga-menu de navigation, hero avec carte de statistiques flottante, footer 5 colonnes) et demandé de "passer à la Phase 1 en transformant le frontend comme sur ces images et en ajoutant toutes les sections comme décrit dans le prompt" — une instruction qui élargit largement la Phase 1 initialement scopée (Accueil+Découvrir uniquement) à l'ensemble des 20 sections du cahier des charges d'un coup.

**Refonte visuelle** (composants partagés, donc appliquée à tout le site) : nouveau `TopBar.tsx` (bandeau contact + réseaux sociaux) ; `Navbar.tsx` réécrit en méga-menu (dropdowns au survol pour « Découvrir Gros-Morne » et « Tourisme », sélecteur de langue FR/HT — encore un simple state local, pas fonctionnel — et icône recherche non branchée), passé de `fixed`/transparent-sur-hero à `sticky` en flux normal (impliquant de retirer le `pt-32` compensatoire de `PageHeader.tsx`, devenu `pt-16`) ; `HeroSection.tsx` entièrement réécrit (un seul visuel par slide au lieu du triple horizontal empilé sur le titre, carte blanche flottante de 6 statistiques à droite, remplaçant l'ancien `StatsBar.tsx` en section séparée — supprimé, plus aucune référence) ; nouveau `Newsletter.tsx` (bandeau d'inscription) inséré avant le footer ; `Footer.tsx` réécrit en 5 colonnes (marque, liens rapides, services, contact) + ligne de liens légaux.

**14 nouvelles pages**, pour couvrir l'intégralité des 20 sections du cahier des charges reçu à la session précédente (jusque-là seules 9 existaient) : `/annuaire` (recherche + filtres par catégorie réellement fonctionnels côté client, sur un tableau de fiches en dur), `/agenda` (mini-calendrier visuel + liste d'événements), `/investir`, `/diaspora`, `/contact` (formulaire avec état de confirmation), `/a-propos`, `/faq` (accordéon `@radix-ui/react-accordion`, déjà une dépendance du projet), `/services-municipaux`, `/vie-associative`, `/sante`, `/education`, `/economie` — ces 4 dernières bâties sur un composant partagé `FicheCardGrid.tsx` (évite de dupliquer le même patron de carte 6 fois) — et les 3 pages légales (`/mentions-legales`, `/confidentialite`, `/conditions-utilisation`) via un composant partagé `LegalContent.tsx`. **Décision assumée** : toutes ces pages restent en contenu statique (tableaux en dur dans le composant, exactement comme les 9 pages préexistantes) plutôt que branchées sur le mécanisme `PageSection` construit en Phase 0 — le câblage backend/admin réel est un chantier à part, section par section, que la demande de Jaslin (portant sur l'apparence et la couverture des sections, pas explicitement sur l'administrabilité immédiate de chacune) ne réclamait pas dans l'instant.

**Un vrai bug de compatibilité trouvé et corrigé** : la version de `lucide-react` installée dans ce projet (`^1.21.0`, sensiblement plus récente que ce que Claude connaît de la librairie) n'exporte plus aucune icône de marque — `Facebook`/`Instagram`/`Youtube`/`Twitter` inexistants dans le module, cassant la compilation TypeScript des nouveaux `TopBar.tsx`/`Footer.tsx`. Corrigé en réutilisant le contournement déjà présent dans l'ancien `Footer.tsx` du projet (icônes génériques `Share2`/`MessageCircle`/`Camera`/`Globe` à la place des logos de marque, confirmé absent de tout ce lucide-react par une recherche programmatique des exports disponibles plutôt qu'un essai-erreur).

**Deux incidents opérationnels sans rapport avec le code applicatif**, tous deux liés à l'environnement d'exécution plutôt qu'au projet : (1) les commandes `npm install` ont pris un temps anormalement long dans cet environnement (jusqu'à 13 minutes pour l'installation initiale du backend), remonté explicitement à Jaslin qui s'en était étonné ("POURQUOI TOUT CE TEMPS"). (2) un processus `next dev -p 3010` zombie (survivant à un arrêt de tâche de la session précédente — `TaskStop` ne tue pas toujours le process enfant `next` que `npm run dev` lance sur Windows) bloquait le port 3010 au redémarrage du serveur ; identifié via `wmic process where "name='node.exe'" get processid,commandline` puis tué avec `taskkill`.

**Correction d'un fait erroné documenté par erreur en Phase 0** : contrairement à ce qui avait été écrit (dans `context/CONTEXT.md`, `context/HISTORY.md` et le fichier de plan éphémère d'`EnterPlanMode`), le dossier `gros-morne` n'était pas "jamais commité" — `git log` a révélé qu'il l'était déjà, par Jaslin lui-même le 2026-06-25, noyé sans mention dans un commit au message consacré à BANKA (`16b4eb4`, probablement un `git add -A` un peu large ce jour-là). Corrigé dans `CONTEXT.md`/`HISTORY.md`. Conséquence pratique, signalée à Jaslin : le déplacement `racine → frontend/` + nouveau `backend/` de la Phase 0 apparaît dans `git status` comme des suppressions + fichiers non suivis plutôt que des renommages propres, tant que Jaslin ne le committe pas lui-même (rien committé par Claude durant cette session, donc rien de perdu, juste un arbre de travail à committer consciemment).

**Vérifié en conditions réelles** : `tsc --noEmit` propre côté frontend (après correction des imports d'icônes). Playwright (headless, pas d'affichage disponible dans cet environnement) sur un script couvrant les 24 pages du site (9 préexistantes + 14 nouvelles + accueil) — chacune chargée avec `waitUntil: networkidle`, vérifiée en HTTP 200 avec un titre de page non vide, aucune erreur console/page/HTTP interceptée sur l'ensemble du parcours — plus des interactions réelles ciblées : méga-menu affiché au survol puis navigation effective vers `/personnalites`, accordéon FAQ qui révèle sa réponse au clic, filtre par catégorie de l'Annuaire qui réduit effectivement la liste affichée, formulaire de contact qui affiche sa confirmation après soumission. Captures d'écran desktop pleine page, mobile (390px) et de la page Annuaire passées en revue visuellement : rendu cohérent avec les maquettes fournies tout en conservant l'identité visuelle forte déjà présente sur le site (vert/or, typographie bold, cartes à accent). 29/29 vérifications automatisées passées.

**Reste à faire, documenté et assumé comme tel** (pas dans le périmètre de cette session) : câblage backend/admin réel section par section (nouvelle Phase 2, révisée en conséquence dans `PLAN.md`) ; recherche et carte interactive réelles (actuellement décoratives) ; sélecteur de langue FR/HT non fonctionnel (état local uniquement) ; contenu des 14 nouvelles pages toujours provisoire/représentatif, à valider avec Jaslin avant toute mise en production ; vulnérabilités npm frontend préexistantes toujours non traitées (signalées en Phase 0, inchangées).

### SMART GROS-MORNE : Phase 0 livrée (fondations backend/admin)

**Contexte :** suite à la réception du cahier des charges et du phasage validé (entrée précédente), Jaslin a dit "phase 0" pour démarrer. Passage par EnterPlanMode (recherche des conventions du reste du portefeuille via un agent Explore sur GESCOM/POSTA/OTELA/LAKAY/REYINYON, plan écrit et approuvé) avant tout code, vu l'ampleur (nouveau backend + réorganisation structurelle + admin complet).

**Réorganisation structurelle :** le frontend Next.js existant (à la racine du projet, seul projet du portefeuille dans ce cas) déplacé vers `frontend/`, nouveau `backend/` créé à côté. Ports attribués : 4010 (backend) / 3010 (frontend), suite logique de la séquence du portefeuille (4001-4008/3001-3009 déjà pris, NEXORA sur 4009/3011 hors de `livrables/applications/`). **Correction découverte en fin de session** : contrairement à ce qui avait été supposé (et écrit dans le plan), le dossier `gros-morne` n'était pas "jamais commité" — il était en fait déjà committé par Jaslin lui-même le 2026-06-25, noyé sans mention dans un commit `16b4eb4` au message consacré à BANKA (probablement via un `git add -A` global). Le déplacement apparaît donc dans `git status` comme des suppressions + de nouveaux fichiers non suivis plutôt que des renommages propres — non destructif (rien committé par Claude durant cette session, tout reste dans l'arbre de travail), mais à committer consciemment plutôt que par un `git add -A` réflexe pour ne pas perdre le lien de renommage dans l'historique.

**Backend** (Express 4 + TypeScript + Prisma v5 + PostgreSQL, layout modules par domaine façon POSTA/OTELA) : auth JWT access 15 min + refresh 30 jours en cookie httpOnly, refresh haché SHA-256 en base (jamais en clair, même durcissement que BANKA/LAKAY/ANTENN/POSTA) ; `validate.middleware.ts` copié du pattern POSTA/OTELA (réassigne bien `body`/`query`/`params` avec le résultat Zod parsé, contrairement à la version GESCOM qui ne fait rien de ça) ; RBAC minimal (`requireRole`, un seul rôle `ADMIN` pour l'instant, extensible plus tard comme ASSOCOTISE) ; upload disque local (multer, pattern REYINYON, dossier `uploads/` à la racine du projet hors de `backend/src`, 10 Mo max, images + PDF). Modèle Prisma central : `PageSection`/`PageSectionTraduction`, le mécanisme de contenu générique et multilingue (FR/HT, confirmé par Jaslin via AskUserQuestion) promis par le cahier des charges — une ligne par langue, contrainte unique `(pageSectionId, locale)`, pensé pour que les Phases 1-5 y branchent tout texte libre sans nouvelle table à chaque fois. Décision assumée de ne pas ajouter de champ `type`/JSON générique dessus tant qu'un vrai besoin de bloc structuré n'apparaît pas.

**Frontend admin** : `/admin/login`, `/admin` (accueil, pas de fausses statistiques), `/admin/contenu` (CRUD complet du mécanisme générique, formulaire bilingue FR/HT à onglets). Design system porté depuis GESCOM/ANTENN (`Badge`/`EmptyState`/`Modal`/`PageToolbar`/`StatCard`, copiés à l'identique) et scopé sous une classe `.gm-admin` dans `globals.css` — décision clé pour ne jamais toucher au CSS du site public existant (notamment `.card-hover`, déjà utilisé par 9 sections publiques avec un effet différent de celui du design system GESCOM). Auth frontend : store zustand + verrou de rafraîchissement partagé (`refreshPromise` dans `lib/api.ts`), pattern LAKAY, adapté puisque `/auth/login` ne renvoie l'admin que dans le corps (l'access token part en cookie httpOnly) — `login()` enchaîne sur `refreshAccessToken()` pour obtenir la copie en mémoire nécessaire à l'en-tête `Authorization`.

**Bug de dépendance trouvé et corrigé en cours de route** : `multer` mis à jour en 2.2.0 (1.x listé par npm avec des CVE connues) puis suppression de `@types/multer` en supposant que multer 2.x embarque ses propres types — hypothèse fausse, multer 2.x n'a aucune déclaration TypeScript, ce qui a cassé la compilation du serveur (`nodemon` en crash loop). Corrigé en réinstallant `@types/multer` (les types 1.x restent structurellement compatibles avec l'API utilisée). Un deuxième incident sans rapport avec le code applicatif : lancer un deuxième `npm run dev` backend par erreur pendant que le premier redémarrait déjà tout seul (nodemon avait détecté le changement de `package.json` et relancé de lui-même) — le doublon arrêté via `TaskStop` avant qu'il ne rencontre un conflit de port.

**Vérifié en conditions réelles** : API (curl — login, `GET /me`, 401 sans token sur une route admin, création d'un bloc avec traductions FR+HT, lecture publique sans auth, mise à jour, suppression, upload d'image réellement servi via `/uploads/...`) ; navigateur (Playwright, exécuté en headless — pas d'affichage disponible dans cet environnement, 8/8 étapes : redirection `/admin/contenu` → `/admin/login` si non connecté, connexion réelle via le formulaire, création d'un bloc bilingue visible dans le tableau, **persistance confirmée après un vrai rechargement de page** (pas juste un état React), modification, suppression, non-régression des pages publiques `/` et `/histoire` toujours rendues correctement). `tsc --noEmit` propre backend et frontend.

**Laissé volontairement de côté** (signalé à Jaslin, pas traité) : vulnérabilités npm résiduelles côté frontend, préexistantes au projet hérité (next 16.2.9, postcss, sharp, js-yaml, brace-expansion) — `npm audit fix --force` proposerait un bump vers next 16.2.12, jugé hors périmètre d'une session de fondations et méritant son propre test plutôt qu'un correctif de dernière minute.

Base remise à un état de seed propre (migration + un seul compte admin, sans donnée de test) après vérification, serveurs arrêtés. Comptes seed : `admin@grosmorne.ht` / `GrosMorne@123`. Prochaine étape validée avec Jaslin : Phase 1 (Accueil + Découvrir — migration du contenu en dur existant vers `PageSection`), pas encore démarrée.

### SMART GROS-MORNE : découverte du projet existant + réception du cahier des charges complet

**Découverte :** dossier `livrables/applications/gros-morne/` déjà présent (Next.js 16 + Tailwind 4 + Radix UI + Framer Motion), jamais documenté dans CONTEXT.md/HISTORY.md ni commité, même pattern que SHOPAY/POSTA découverts le 2026-07-07. 9 pages statiques déjà construites (accueil, histoire, géographie, culture, personnalités, tourisme, communauté, galerie, actualités), contenu en dur dans les composants, aucun backend/base de données/admin.

**Cahier des charges reçu :** Jaslin a fourni la structure complète du site vitrine de la commune de Gros-Morne, commandé par un client réel — 20 sections (Accueil, Découvrir Gros-Morne, Tourisme, Économie, Investir, Annuaire, Actualités, Agenda, Galerie, Diaspora, Services Municipaux, Vie Associative, Éducation, Santé, Contact, À propos, FAQ, Mentions légales, Politique de confidentialité, Conditions d'utilisation), avec exigence transversale que tout contenu (texte/image/vidéo/document) soit administrable depuis un tableau de bord sans intervention développeur, plus recherche/filtres, SEO et performance.

**Décisions prises avec Jaslin (AskUserQuestion) avant tout code :** documenter puis planifier plutôt que coder directement ; continuer sur le frontend existant plutôt que repartir de zéro ; backend maison Express + Prisma + PostgreSQL (cohérent avec BANKA/GESCOM/LAKAY/OTELA...) plutôt qu'un CMS headless tiers ; projet confirmé commandé par un client réel (pas une initiative perso).

**Livré cette session :** cahier des charges complet + phasage proposé sauvegardés dans `livrables/applications/gros-morne/PLAN.md` (Phase 0 fondations backend/admin/contenu générique multilingue, Phase 1 Accueil+Découvrir, Phase 2 Tourisme+Annuaire, Phase 3 Actualités+Agenda+Galerie, Phase 4 sections à structure "fiche" restantes, Phase 5 pages statiques/SEO/perf). Ajout du projet à la liste des projets actifs de CONTEXT.md. **Aucun code écrit** — Phase 0 en attente de validation de Jaslin avant de démarrer.
...
**Contexte :** sur « CONTINUE », il ne restait plus qu'un seul chantier réellement réalisable — les paiements en ligne (#10) demeurant bloqués sur des credentials Digicel/MonCash externes que Jaslin n'a pas fournis. Pas de clarification nécessaire cette fois (un seul choix possible), contrairement aux tours précédents où plusieurs options coexistaient.

**Livré :** quatre pages Server Component statiques, mêmes conventions SEO que `/contact` (metadata dédiée par page) : `/mentions-legales`, `/cgu` (conditions générales d'utilisation et de vente), `/confidentialite`, `/faq`. Contenu juridique volontairement générique — chaque fichier porte un commentaire explicite rappelant qu'il s'agit d'un modèle à faire relire par un vrai juriste avant tout usage réel, décision déjà actée lors de la clarification d'ampleur de la session vitrine (2026-07-22). La page confidentialité reflète fidèlement ce que l'application collecte réellement (nom/téléphone/email via le modèle `Client`, aucun tracker tiers dans le codebase) plutôt qu'une liste générique copiée d'ailleurs. La FAQ reste honnête sur l'état réel du produit : la question sur les moyens de paiement précise que le règlement s'effectue **actuellement sur place**, sans promettre un paiement en ligne qui n'est pas encore actif (cohérent avec le point #10 toujours bloqué) — plutôt que de rédiger un contenu marketing déconnecté de ce qui est réellement livré.

**FAQ en accordéon** (`FaqAccordion.tsx`) : seul îlot client de la page, même principe que `Lightbox.tsx` sur la fiche établissement — le contenu textuel des questions/réponses reste présent dans le HTML server-rendered pour le référencement, seule l'interaction d'ouverture/fermeture est côté client. Les 4 pages ajoutées au footer public (`(public)/layout.tsx`, jusque-là un simple texte sans lien) et à `app/sitemap.ts`.

**Vérifié en conditions réelles :**
- `tsc --noEmit` propre.
- Navigateur (Playwright, chromium en cache) : **15/15 vérifications** — titre et balise `<h1>` corrects sur les 4 pages, les 4 liens du footer présents et fonctionnels, accordéon FAQ vérifié (première question ouverte par défaut, clic sur une autre question bascule bien son contenu), contenu de la réponse sur le paiement confirmé fidèle à l'état réel du produit (mention "sur place" présente). Deux faux échecs de script corrigés en cours de route (sélecteur de bouton ne correspondant pas au texte réel de la question, puis une vérification qui lisait le texte d'une section entre-temps refermée par l'accordéon à sélection unique — les deux corrigés dans le script de test, pas dans le produit).
- Aucun changement backend ce chantier (pages 100 % statiques) — pas de migration, pas de reset de base nécessaire. Playwright installé `--no-save` puis désinstallé, serveur frontend arrêté par PID ciblé.

**Cahier des charges SaaS Multi-Hôtel de 22 points désormais entièrement traité ou explicitement documenté comme bloqué.** Seul le point #10 (paiements en ligne réels, Digicel/MonCash) reste en attente des credentials de Jaslin — hors de portée sans son intervention. Tests automatisés toujours absents sur l'ensemble du portefeuille. Rien commité en git au moment de l'écriture de cette entrée.

---

## 2026-07-23

### OTELA : Check-in signé + Notifications WhatsApp (#8/#17) livrés

**Contexte :** sur « vas y » après un premier « CONTINUE » resté ambigu (deux chantiers restaient possibles), clarification par AskUserQuestion : Jaslin a choisi Check-in signé + WhatsApp plutôt que les pages légales. Plan écrit et validé (EnterPlanMode).

**Signature au check-in (#8) :** `Reservation.signatureUrl`/`signatureDate` ajoutés (migration additive `signature_checkin`), champs directs sur `Reservation` comme `reference` (un seul signataire par séjour, pas de modèle séparé). Nouveau sous-dossier `uploads/signatures` dans `upload.middleware.ts` (même pattern multer que logos/photos — filtre image sans SVG, nom de fichier généré). Le check-in (`reception.service.ts::checkin`) exige désormais la signature : refusé en 400 sans fichier — cohérent avec l'intention du document, le check-in EST le moment de la signature, pas une étape à sauter. Frontend : nouveau `components/reception/SignaturePad.tsx` (canvas, Pointer Events unifiant souris/tactile pour une tablette de comptoir ; mise à l'échelle explicite des coordonnées pointeur→canvas, la résolution interne du canvas différant de sa taille CSS étirée en `w-full` — sans ça le tracé décale) et `CheckinModal.tsx` (récapitulatif client/chambre, bouton "Confirmer" désactivé tant qu'aucun trait n'est dessiné). Sur `/reception`, le bouton "Check-in" ouvre ce modal ; une arrivée déjà signée affiche un badge "Signé ✓" à la place du bouton.

**Notifications WhatsApp (#17), même philosophie qu'ASSOCOTISE (relances) et REYINYON (invitation) :** jamais d'envoi automatisé — un lien `wa.me` pré-rempli est ouvert par le navigateur, l'action reste déclenchée par un humain. Nouveau `frontend/src/lib/whatsapp.ts::construireLienWhatsApp` (normalisation du numéro haïtien, préfixe `509` si absent). Bouton "Confirmer par WhatsApp" sur l'écran de confirmation public (`ReserverClient.tsx`) et sur `/ma-reservation` — client-déclenché, non journalisé. Bouton "Rappel WhatsApp" sur les arrivées du jour de `/reception` — personnel-déclenché, journalisé via un nouvel endpoint `POST /reservations/:id/whatsapp-log` (nouvelle valeur `WHATSAPP_ENVOYE` ajoutée à `ActionAudit` dans `audit.service.ts`), fire-and-forget après l'ouverture du lien pour ne jamais bloquer l'UX. La trace atteste que la démarche a été déclenchée, pas que le message a été reçu — même nuance déjà écrite explicitement dans l'interface d'ASSOCOTISE.

**Vérifié en conditions réelles :**
- `tsc --noEmit` propre backend et frontend.
- API (curl, backend sur la vraie base Postgres `otela_db`) : check-in refusé (400, message clair) sans fichier de signature ; check-in réussi avec un vrai PNG, fichier ensuite retéléchargé et comparé **octet pour octet identique** à l'original ; upload d'un fichier `text/plain` en guise de signature rejeté (400, filtre MIME) ; double check-in toujours refusé (409) par la garde chambre déjà existante (`chambre.statut !== DISPONIBLE`), sans modification nécessaire — confirme que cette garde protégeait déjà nativement le nouveau flux ; `POST /whatsapp-log` journalise une entrée `WHATSAPP_ENVOYE` correctement consultable via `GET /audit`.
- Navigateur (Playwright, chromium en cache réutilisé) : signature **réellement tracée à la souris** sur le canvas (mouvements réels, pas un clic simulé) avec vérification que le bouton "Confirmer" reste désactivé avant le tracé et s'active après ; check-in complet de bout en bout depuis `/reception` ; badge "Signé" affiché ensuite à la place du bouton. Parcours de réservation publique complet (recherche → sélection → formulaire → confirmation) jusqu'au bouton WhatsApp de confirmation.
- **Incident de vérification (pas un bug produit) :** le popup `wa.me` n'était pas détectable par `context.waitForEvent('page')` faute de réseau vers WhatsApp dans ce sandbox (même limitation déjà rencontrée avec `next/font/google` plus tôt dans le portefeuille) — un premier test concluait donc à tort à un échec. Confirmé non-bug en instrumentant `window.open` directement dans la page (`page.evaluate`) pour capturer l'URL construite sans tenter de naviguer réellement : lien et message corrects dans les deux cas (rappel personnel avec numéro `50937123456` correctement préfixé, confirmation client avec référence/dates/chambre exactes).
- Nettoyage : fichiers de signature de test supprimés (`uploads/signatures/`, dossier entièrement retiré puisque recréé automatiquement par `mkdirSync` au prochain démarrage), base remise à un état de seed propre (`prisma migrate reset` + reseed), serveurs arrêtés par PID ciblé, script et dépendance Playwright retirés (`package.json`/`package-lock.json` confirmés intacts, seul `exceljs` du chantier précédent y figure).

**Reste du cahier des charges SaaS Multi-Hôtel (non traité) :** mentions légales/CGU/confidentialité/FAQ, paiements en ligne réels (#10, bloqués sur credentials Digicel/MonCash). Tests automatisés toujours absents. Rien commité en git au moment de l'écriture de cette entrée.

---

## 2026-07-22 (suite 3)

### OTELA : Dashboard graphique + Exports Excel (#6/#15) livrés

**Contexte :** sur « CONTINUE », clarification (AskUserQuestion) sur le prochain chantier parmi 4 options — Jaslin a choisi Dashboard + Exports Excel plutôt que signature check-in+WhatsApp ou pages légales. Plan écrit et validé (EnterPlanMode). Skill `dataviz` chargée avant d'écrire le moindre code de graphique, comme requis par ses règles de déclenchement.

**Décision de conception centrale :** pas de nouvelle bibliothèque de graphiques (recharts, etc.) — cohérent avec le reste du portefeuille où toute l'UI (Badge, Modal, StatCard) est construite à la main plutôt qu'importée. Deux composants SVG légers `LineChart`/`BarChart` suivant les spécifications de la skill (marques 2px, halo au survol, info-bulle, étiquettes sélectives, table de données en repli) mais **réutilisant les tokens de couleur déjà existants d'OTELA** (`--color-primary-3` bleu, `--color-accent` or) plutôt que la palette de substitution fournie par la skill — la marque a déjà ses propres couleurs établies dans tout le produit, les remplacer aurait cassé la cohérence visuelle. Confirmé qu'aucune page du back-office OTELA n'a de mode sombre (contrairement à d'autres projets du portefeuille) : les graphiques restent clair uniquement. ADR et RevPAR traités comme des `StatCard` (chiffres uniques par période), pas des graphiques — règle de la skill "un chiffre isolé est une tuile, pas un graphique à une barre".

**Backend :** `rapports.service.ts::getOccupationEtRevenu` étendue avec `adrParDevise` (revenu / nuit occupée), `revparParDevise` (revenu / nuit disponible) et `repartitionParType` (regroupement par type de chambre, triée par revenu HTG décroissant). Nouvelle fonction `getSerieJournaliere` (un point par jour calendaire, revenu réparti au prorata des nuits de chaque réservation plutôt qu'attribué en bloc à la date d'arrivée — sinon la courbe de revenu ne corrélerait plus avec celle de l'occupation). Nouveau module `rapports.export.ts` (`exceljs`, ajoutée en dépendance réelle cette fois, contrairement à Playwright qui reste temporaire) : classeur "Résumé" + "Réservations" détaillées, pour un établissement et pour la chaîne. Nouvelles routes `GET /rapports/etablissement/serie`, `GET /rapports/etablissement/export.xlsx`, `GET /rapports/chaine/export.xlsx` (mêmes RBAC que les rapports existants).

**Frontend :** `rapportsStore.ts` étendu (types + `serieJournaliere` + `exporter()` en téléchargement blob authentifié, même pattern que `FactureModal.tsx::telechargerPdf`). Page `/rapports` enrichie : tuiles ADR/RevPAR, courbe de taux d'occupation, barres de revenu quotidien par devise (jamais HTG et USD sur le même graphique), barres horizontales de répartition par type de chambre, bouton "Exporter en Excel". Bouton d'export ajouté à `/chaine` (pas de nouveaux graphiques côté chaîne, hors périmètre décidé — le pilotage graphique cible la direction d'établissement).

**Deux bugs réels trouvés et corrigés en vérification :**
- **Nuit fantôme au check-out.** La boucle de `getSerieJournaliere` bornait sur l'horodatage brut de `dateDepart` au lieu de son jour calendaire tronqué — un check-out en cours de journée faisait compter à tort la nuit du jour de départ comme occupée, pour chaque réservation. Trouvé en vérifiant que la somme de la série journalière égalait exactement l'agrégat existant (7 nuits dans la série contre 5 dans l'agrégat sur les mêmes données de test) ; corrigé en bornant la boucle sur `debutJour(dateDepart)`. Revérifié : somme des nuits et du revenu de la série identique à l'agrégat au HTG près.
- **Nom de fichier accentué invalide.** `Content-Disposition` n'accepte que de l'ASCII sans encodage RFC 5987 — un nom d'établissement comme "OTELA Pétion-Ville" produisait un en-tête cassé (confirmé en tête de réponse curl). Corrigé par une translittération ASCII (`normalize('NFD')` + suppression des marques diacritiques) avant construction du nom de fichier.

**Vérifié en conditions réelles :**
- `tsc --noEmit` propre backend et frontend.
- API (curl, backend sur la vraie base Postgres `otela_db`) : données de test insérées directement via Prisma (l'API rejette à raison toute date d'arrivée passée pour un vrai client, donc impossible de simuler un historique par ce chemin) — agrégat (`adrParDevise`/`revparParDevise`/`repartitionParType`) vérifié exact à la main (43500 HTG / 5 nuits = 8700 ADR, 43500 / 80 nuits-chambre disponibles = 543,75 RevPAR, Suite avant Standard par revenu décroissant) ; série journalière recoupée nuit par nuit et HTG par HTG avec l'agrégat après correction du bug. Les deux classeurs Excel téléchargés puis **relus programmatiquement avec `exceljs`** (pas seulement code 200 et Content-Type) : feuilles "Résumé"/"Réservations" présentes avec les bonnes valeurs et les bonnes lignes.
- Navigateur (Playwright, chromium en cache, package installé `--no-save` puis désinstallé) : **12/12 vérifications** — tuiles ADR/RevPAR visibles, les trois graphiques rendus avec de vraies données, info-bulle déclenchée au survol de la courbe, bascule "Voir les données" affichant la table, **téléchargement réel** d'un fichier Excel non vide (8065 octets, taille identique à celle obtenue via curl) déclenché par le bouton, bouton d'export présent sur `/chaine`, aucune erreur console. Capture d'écran pleine page de `/rapports` revue visuellement (dernière étape de la skill dataviz) : rendu propre, aucune étiquette qui se chevauche, cohérent avec le design system existant (couleurs, cartes, typographie).
- Nettoyage : données de test supprimées par `prisma migrate reset` + reseed, serveurs arrêtés par PID ciblé, script et dépendance Playwright retirés (seul `exceljs` reste, en dépendance permanente confirmée par `git diff package.json`).

**Reste du cahier des charges SaaS Multi-Hôtel (non traité) :** signature numérique au check-in (#8), notifications SMS/WhatsApp (#17), mentions légales/CGU/confidentialité/FAQ, paiements en ligne réels (#10, bloqués sur credentials Digicel/MonCash). Tests automatisés toujours absents. Rien commité en git au moment de l'écriture de cette entrée.

---

## 2026-07-22 (suite 2)

### OTELA : Site vitrine + Avis clients + SEO (#5/#19) livrés

**Contexte :** sur « CONTINUE AVEC SITE VITRINE AVIS + SEO », clarification préalable (AskUserQuestion) sur l'ampleur : Jaslin a choisi les **pages à valeur réelle** plutôt que les 10 pages du cahier des charges au complet — pas de mentions légales/CGU/confidentialité/FAQ cette session (texte juridique générique, hors périmètre produit). Plan écrit et validé (EnterPlanMode) avant codage.

**Modèle de données (migration additive `avis`) :** `Avis` — un par séjour (`reservationId @unique`), `note` 1-5, `commentaire`, `visible` (modération par masquage, jamais suppression — même philosophie que le reste du portefeuille), `reponseDirection`/`reponseDate` optionnels (réponse de la direction, pattern Booking/TripAdvisor).

**Décision de conception centrale :** un avis n'est jamais soumis via un formulaire ouvert. `avis.service.ts::soumettreAvis` réutilise telle quelle `reservations.service.ts::getReservationPublique(reference, email)` — même garde anti-énumération déjà en place (message identique si référence ou email ne correspond pas), rejette 400 si le séjour n'est pas `TERMINEE`, 409 si un avis existe déjà pour cette réservation. Nouveau `avisLimiter` (10/heure) sur ce point d'écriture public.

**Backend :** nouvel endpoint public `GET /etablissements/:id/vitrine` (`etablissements.service.ts::getEtablissementVitrine`) — chambres+tarifs+photos+avis visibles+moyenne en une seule requête composée (2 appels Prisma en parallèle), pour que la page serveur n'ait besoin que d'un seul fetch. Nouveau module `avis/` : RBAC lecture `ADMINISTRATEUR_ETABLISSEMENT`/`ADMINISTRATEUR_CHAINE`/`PROPRIETAIRE` (lecture seule pour ce dernier, cohérent avec le reste du portefeuille), modération (masquer/répondre) réservée à la direction établissement/chaîne. Pas de journal d'audit sur ce module (cohérent avec menage/spa/maintenance — réservé aux actions sensibles argent/sécurité/identité). `reservations.service.ts::getReservationPublique` étendue avec `avis: { select: { id: true } }` pour que `/ma-reservation` sache si un avis existe déjà sans requête supplémentaire.

**Frontend — restructuration SEO obligatoire :** `'use client'` interdit `export const metadata` dans le même fichier. `/reserver` et `/ma-reservation` scindées en Server Component (`page.tsx`, metadata) + Client Component (`ReserverClient.tsx`/`MaReservationClient.tsx`, logique strictement inchangée sauf ajouts). `ReserverClient` lit désormais `?etablissementId=` pour pré-remplir le formulaire (même pattern que le `?ref=` déjà utilisé sur ma-reservation). `MaReservationClient` : si le séjour est `TERMINEE` et qu'aucun avis n'existe encore, formulaire "Laisser un avis" (étoiles 1-5 + commentaire) inline.

**Nouvelle page `/etablissements/[id]`** — Server Component pur, SSR réel (pas juste des balises) : `generateMetadata` dynamique par établissement (title/description/Open Graph), JSON-LD `schema.org/Hotel` (avec `aggregateRating` si des avis existent), galerie de chambres par type avec un seul îlot interactif `Lightbox.tsx` (agrandissement photo au clic, le reste du contenu textuel reste server-rendered pour le SEO), coordonnées + lien carte, section avis (moyenne en étoiles + liste avec réponse direction éventuelle), CTA "Réserver ici" vers `/reserver?etablissementId=`. Les cartes établissement de `/reserver` pointent désormais vers cette fiche au lieu de faire défiler vers le formulaire de recherche.

**Nouvelle page `/contact`** — Server Component, coordonnées réelles de tous les établissements (adresse/téléphone/email, liens `tel:`/`mailto:`), pas de formulaire avec backend (cohérent avec le reste du portefeuille — aucune automatisation d'envoi ailleurs, ex. relances ASSOCOTISE, invitation WhatsApp REYINYON).

**SEO transversal :** `app/sitemap.ts` (entrées statiques + dynamiques par établissement, `ma-reservation` volontairement exclue) et `app/robots.ts` natifs Next.js App Router, `disallow` sur `/login` et `/ma-reservation`. `ma-reservation` marquée `robots: { index: false }` (page de consultation privée). Nouvelle variable `NEXT_PUBLIC_SITE_URL` dans `.env.local`.

**Back-office — modération avis :** nouvelle page `/avis` (liste avec note/commentaire/date, toggle masquer/afficher, champ réponse), réservée écriture direction/chaîne, lecture seule `PROPRIETAIRE` (contrôles cachés côté UI en plus du 403 serveur). Ajoutée à `Sidebar.tsx` (établissement/chaîne/propriétaire) et `PROTECTED_PREFIXES`.

**Bug réel trouvé et corrigé en vérification :** le fetch de la page établissement utilisait `next: { revalidate: 60 }` (ISR) — un avis fraîchement soumis ou une réponse de la direction restait invisible jusqu'à 60 secondes après soumission, découvert en testant le cycle complet soumission→affichage en navigateur (le test échouait juste après une soumission réelle). Corrigé en `cache: 'no-store'` : la fraîcheur des avis prime sur la charge marginale d'une page peu visitée par établissement.

**Vérifié en conditions réelles :**
- `tsc --noEmit` propre backend et frontend.
- API (curl, backend sur la vraie base Postgres `otela_db`) : cycle complet réservation→check-in→paiement intégral→check-out→soumission d'avis sur une vraie réservation de seed (statut `TERMINEE` confirmé avant soumission) ; avis refusé (400) sur une réservation encore `CONFIRMEE` ; email erroné et référence inexistante renvoyant le même message 404 (anti-énumération) ; double soumission refusée (409) ; note hors 1-5 rejetée (422) ; `GET /etablissements/:id/vitrine` renvoyant bien chambres+photos+avis+moyenne ; `PROPRIETAIRE` 200 en lecture mais 403 en écriture (PATCH) ; masquage d'un avis le faisant réellement disparaître de la vitrine publique (`avisTotal` et liste vérifiés avant/après) ; réponse de la direction persistée avec `reponseDate`.
- Navigateur (Playwright, chromium en cache, package installé `--no-save` puis désinstallé) : **22/22 vérifications** — titre et JSON-LD dynamiques sur la fiche établissement (avec `aggregateRating`), chambres et avis avec réponse visibles, CTA "Réserver ici" naviguant vers `/reserver` avec le formulaire réellement pré-rempli, page `/contact` listant les vrais établissements, `/sitemap.xml` et `/robots.txt` répondant avec le contenu attendu, cycle complet de soumission d'un avis via le vrai formulaire `/ma-reservation` (clic sur les étoiles, texte, envoi) suivi de sa réelle apparition sur la fiche établissement (après le correctif de cache), visibilité côté back-office `/avis`, masquage réel répercuté côté public. Deux faux échecs de script (lecture de `page.url()` et d'un `select` avant la fin de la navigation/hydratation asynchrone) confirmés comme artefacts de test et non des bugs, par une vérification isolée immédiate.
- Nettoyage : base remise à un état de seed propre (`prisma migrate reset` + reseed), serveurs arrêtés par PID ciblé sur le port, script et dépendance Playwright retirés (`package.json`/`package-lock.json` confirmés intacts).

**Reste du cahier des charges SaaS Multi-Hôtel (non traité) :** signature numérique au check-in (#8), notifications SMS/WhatsApp (#17), exports Excel (#15), dashboard avec graphiques (#6), mentions légales/CGU/confidentialité/FAQ (volontairement écartées cette session), paiements en ligne réels (#10, bloqués sur credentials Digicel/MonCash). Tests automatisés toujours absents. Rien commité en git au moment de l'écriture de cette entrée.

---

## 2026-07-22 (suite)

### OTELA : Maintenance (#13) + Inventaire (#14) livrés

**Contexte :** sur « CONTINUE AVEC OTELA », Jaslin a choisi « Maintenance + Inventaire » parmi 4 options proposées (les autres : dashboard+exports Excel, site vitrine+avis, signature check-in+WhatsApp). Plan écrit et validé (EnterPlanMode) avant codage. Le rôle `MAINTENANCE` existait déjà depuis la tranche Fondations SaaS mais n'avait aucun écran à piloter — seule action possible : la bascule DISPONIBLE↔MAINTENANCE d'une chambre, déjà en place dans `chambres.service.ts::basculerMaintenance`.

**Modèle de données (migration additive `maintenance_inventaire`) :** `TicketMaintenance` (chambre **ou** zone commune en texte libre — jamais les deux, invariant Zod ; `priorite` BASSE/NORMALE/HAUTE/URGENTE ; `statut` A_FAIRE/EN_COURS/RESOLU, même nommage que `StatutTacheMenage` ; `bloqueChambre` optionnel) ; `ArticleInventaire`/`MouvementInventaire` (stock opérationnel interne — linge, consommables, produits d'entretien — distinct d'`ArticleMinibar` qui sert la facturation client au folio, pas le suivi de stock).

**Décision de conception centrale (Maintenance) :** signaler un problème ne doit jamais échouer parce que la chambre est occupée. Si `bloqueChambre` est demandé mais que la chambre n'est pas `DISPONIBLE`, le ticket est créé quand même, seul le flag est ramené à `false` (documenté, pas une erreur bloquante). Résolution d'un ticket bloquant : ne restaure `DISPONIBLE` que si (a) la chambre est encore `MAINTENANCE` et (b) aucun **autre** ticket encore ouvert avec `bloqueChambre:true` ne vise la même chambre — sinon un second problème en cours serait écrasé. Compare-and-swap Prisma à chaque transition (même pattern que `menage.service.ts::updateTache`).

**Décision de conception centrale (Inventaire) :** mouvements ENTREE/SORTIE/AJUSTEMENT. SORTIE en compare-and-swap (`quantiteStock >= quantite`, rejet 409 si insuffisant, jamais de stock négatif — même pattern que la garde de stock GESCOM/SHOPAY). AJUSTEMENT prend une valeur **absolue** (le nouveau stock exact après inventaire physique), pas un delta — évite toute ambiguïté de signe côté formulaire. `MouvementInventaire.stockApres` figé à chaque mouvement pour un historique lisible sans recalcul.

**RBAC :** signalement de ticket ouvert à RECEPTION/MENAGE/MAINTENANCE/direction (n'importe quel opérationnel peut repérer un problème) ; gestion (statut, assignation, résolution) réservée à MAINTENANCE et la direction — même trio que la bascule `/chambres/:id/maintenance` existante. Inventaire : usage quotidien (lecture + mouvements) ouvert à MENAGE/MAINTENANCE/direction ; création/gestion du catalogue d'articles réservée à la direction. Pas de journal d'audit sur ces deux modules — cohérent avec menage/spa/room-service, l'audit restant réservé aux actions sensibles (argent, sécurité, identité).

**Vérifié en conditions réelles :**
- `tsc --noEmit` propre backend et frontend dès l'écriture.
- API (curl, backend sur la vraie base Postgres `otela_db`) : signalement avec `bloqueChambre` sur chambre disponible → bascule réelle en MAINTENANCE ; second signalement bloquant sur la même chambre déjà en maintenance → flag forcé à `false` (pas d'erreur) ; **scénario complet de la garde anti-écrasement** — chambre remise disponible manuellement, second ticket vraiment bloquant créé, les deux tickets coexistent ouverts sur la même chambre, résolution du premier ne restaure PAS (vérifié en relisant l'état réel de la chambre en base), résolution du second (dernier bloquant) restaure bien `DISPONIBLE` ; double-résolution rejetée en 409 ; ticket sur zone commune sans chambre ; validation Zod rejetant chambre+zone ensemble et ni l'une ni l'autre (422) ; RBAC 403 sur mise à jour par MENAGE ; admin chaîne filtrant par `etablissementId` en query ; assignation à un employé MAINTENANCE. Inventaire : RBAC 403 sur création d'article par MENAGE ; création + doublon de nom rejeté en 409 ; ENTREE (0→50), SORTIE (50→35), SORTIE en stock insuffisant rejetée 409, AJUSTEMENT (valeur absolue → 100) ; historique des mouvements avec `stockApres` cohérent et employé correctement attribué à chaque ligne.
- Navigateur (Playwright, chromium en cache réutilisé, package installé `--no-save` puis désinstallé) : connexion RECEPTION → création réelle d'un ticket zone commune via le formulaire, visible dans la liste, RECEPTION sans contrôle d'assignation visible ; connexion MAINTENANCE → contrôle d'assignation visible, avancement de statut cliqué ; connexion MENAGE → page Inventaire visible mais sans bouton "Nouvel article" (réservé direction) ; connexion direction → bouton "Nouvel article" visible, article créé et visible, mouvement ENTREE enregistré via le formulaire avec badge "Stock bas" affiché correctement une fois le stock sous le seuil. 16/16 vérifications automatisées passées.

**Incident d'environnement (pas un bug produit) :** `next/font/google` (Inter) nécessite une résolution réseau pour télécharger la police au démarrage du serveur de dev, réseau indisponible dans ce sandbox — bloquait toute page en 500. `layout.tsx` temporairement modifié (police retirée) le temps du test navigateur, puis **restauré à l'identique** (diff vide confirmé par `git diff`) avant la fin de la session. Un `rm -rf .next` sur un cache figé (404 sur toutes les routes) avait par ailleurs supprimé le cache de polices précédemment téléchargé, révélant le problème.

**Nettoyage :** base remise à un état de seed propre (`prisma migrate reset` + reseed), serveurs backend/frontend arrêtés par PID ciblé sur le port, script de vérification et dépendance Playwright retirés (`package.json`/`package-lock.json` confirmés intacts par `git status`).

**Reste du cahier des charges SaaS Multi-Hôtel (non traité) :** signature numérique au check-in (#8), notifications SMS/WhatsApp (#17), exports Excel (#15), dashboard avec graphiques (#6), site vitrine 10 pages + avis + SEO (#5/#19), paiements en ligne réels (#10, bloqués sur credentials Digicel/MonCash). Tests automatisés toujours absents. Rien commité en git au moment de l'écriture de cette entrée (à la main de Jaslin).
