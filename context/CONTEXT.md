...
- **BANKA** : système de gestion bancaire complet. Stack : Next.js App Router + Express 4 + TypeScript + Prisma v5 + PostgreSQL. Ports : 4001 (backend) / 3001 (frontend). Modules livrés : Tableau de bord, Clients, Comptes (9 types : EPARGNE, COURANT, TERME, JOINT, MICRO_EPARGNE, TONTINE, RETRAITE, JEUNESSE, CREDIT), Transactions (dépôt/retrait/virement), Caisse (sessions journalières avec auto-liaison des transactions à la session ouverte), Crédits & Prêts (amortissement dégressif/constant/in fine, décaissement, remboursement), Rapports (journalier, PAR 30/90, impayés), Journal d'audit, Utilisateurs, Agences (avec compteur d'employés RH), Mandats & Procurations (droits délégués par compte, possibilité de créer un mandataire externe non-client), Administration (10 paramètres configurables), RH complet (postes, employés avec agence assignée et compte système lié, compte bancaire lié et mode règlement VIREMENT_BANKA/ESPECES, contrats, congés, bulletin de paie workflow BROUILLON→VALIDÉ→PAYÉ, avances sur salaire déduites à la paie, déduction automatique versement mensuel prêt en cours, éléments variables PRIME/BONUS/INDEMNITE/HEURE_SUP/RETENUE). Fonctionnalités transversales : RBAC 7 rôles, audit log complet toutes mutations, pénalités de retard automatiques depuis config, génération PDF dossier crédit, salaire payé via Transaction réelle (visible dans le relevé de compte de l'employé), formatage compact des montants (K/M/Md HTG) sur tous les dashboards, blocage des transactions tant que la caisse n'est pas ouverte (middleware requireCaisseOuverte, bypass automatique pour le siège). En cours de développement actif. Modules restants : Comptabilité (plan comptable, journaux, grand livre, bilan).
