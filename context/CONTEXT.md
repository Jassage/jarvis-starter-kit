...
- **GESCOM** : ERP commercial pour un client signé (entreprise avec 1 boutique détail + 1 entrepôt grossiste, stocks séparés, devise HTG, 5-20 utilisateurs). Stack : Next.js App Router + Express 4 + TypeScript + Prisma v5 + PostgreSQL. Ports : 4002 (backend) / 3003 (frontend). Phase 0 (socle) + Phase 1 (Stock/Produits) livrées le 2026-06-30. Inclut : schéma de données complet (Stock multi-emplacement + Ventes + Achats + Comptabilité), auth JWT + RBAC 5 rôles, audit log, dashboard premium redesigné (hero gradient, KPI cards gradient, répartition par emplacement, timeline mouvements, 100% responsive mobile/desktop avec sidebar drawer), pages Produits (CRUD + alertes seuil) et Stock (ajustement atomique + mouvements + alertes). Design : thème vert, police Inter, sidebar fixe desktop/drawer mobile avec icônes lucide-react, header avec dropdown utilisateur. Compte démo : admin@gescom.ht / Admin@123. Prochaines phases : Ventes/Facturation (Phase 2), Achats/Fournisseurs (Phase 3), Transferts (Phase 4), Comptabilité (Phase 5), Rapports (Phase 6). Nom de code provisoire, renommable.
