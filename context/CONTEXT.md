...
- **BANKA** : système de gestion bancaire complet. Stack : Next.js App Router + Express 4 + TypeScript + Prisma v5 + PostgreSQL. Ports : 4001 (backend) / 3001 (frontend). Modules livrés : Tableau de bord, Clients, Comptes (9 types : EPARGNE, COURANT, TERME, JOINT, MICRO_EPARGNE, TONTINE, RETRAITE, JEUNESSE, CREDIT), Transactions (dépôt/retrait/virement), Caisse (sessions journalières), Crédits & Prêts (amortissement dégressif/constant/in fine, décaissement, remboursement), Rapports (journalier, PAR 30/90, impayés), Journal d'audit, Utilisateurs, Agences, Mandats & Procurations (droits délégués par compte), Administration (10 paramètres configurables). Fonctionnalités transversales : RBAC 7 rôles, audit log complet toutes mutations, pénalités de retard automatiques depuis config, génération PDF dossier crédit. En cours de développement actif.
