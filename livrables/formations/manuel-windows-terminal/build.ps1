# Build du manuel Windows Terminal/CMD/PowerShell : assemble les chapitres Markdown et genere HTML + PDF + DOCX.
...
$htmlOut = Join-Path $exportDir "manuel-windows-terminal.html"
$docxOut = Join-Path $exportDir "manuel-windows-terminal.docx"
$pdfOut  = Join-Path $exportDir "manuel-windows-terminal.pdf"
...
pandoc @cheminsAbsolus `
  ...
  --metadata pagetitle="Manuel complet Windows Terminal, CMD et PowerShell" `
...
pandoc @cheminsDocx `
  ...
  --toc `
  --toc-depth=2 `
  --metadata title="Manuel complet Windows Terminal, CMD et PowerShell" `
  --metadata author="Jaslin Occius" `
  --output="$docxOut"
...
if (Test-Path $nodeScript) {
  node "$nodeScript" "$htmlOut" "$pdfOut" "Jaslin Occius - Manuel Windows Terminal & PowerShell"
} else {
