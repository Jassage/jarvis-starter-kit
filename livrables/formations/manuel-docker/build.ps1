# Build du manuel Docker : assemble les chapitres Markdown et genere HTML + PDF + DOCX.
...
$htmlOut = Join-Path $exportDir "manuel-docker.html"
$docxOut = Join-Path $exportDir "manuel-docker.docx"
$pdfOut  = Join-Path $exportDir "manuel-docker.pdf"
$htmlOutPandoc = "export/manuel-docker.html"
$docxOutPandoc = "export/manuel-docker.docx"
...
pandoc @cheminsAbsolus `
  ...
  --metadata pagetitle="Docker de zero a expert" `
...
pandoc @cheminsDocx `
  ...
  --toc `
  --toc-depth=2 `
  --metadata title="Docker de zero a expert" `
  --metadata author="Jaslin Occius" `
...
if (Test-Path $nodeScript) {
  node "$nodeScript" "$htmlOut" "$pdfOut" "Jaslin Occius - Docker de zero a expert"
} else {
