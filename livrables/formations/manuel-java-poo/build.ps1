# Build du manuel Java POO : assemble les chapitres Markdown et genere HTML + PDF + DOCX.
...
$htmlOut = Join-Path $exportDir "manuel-java-poo.html"
$docxOut = Join-Path $exportDir "manuel-java-poo.docx"
$pdfOut  = Join-Path $exportDir "manuel-java-poo.pdf"
...
pandoc @cheminsAbsolus `
  ...
  --metadata pagetitle="Manuel complet Java - Programmation Orientee Objet" `
...
pandoc @cheminsDocx `
  ...
  --toc `
  --toc-depth=2 `
  --metadata title="Manuel complet Java - Programmation Orientee Objet" `
  --metadata author="Jaslin Occius" `
  --output="$docxOut"
...
if (Test-Path $nodeScript) {
  node "$nodeScript" "$htmlOut" "$pdfOut" "Jaslin Occius - Manuel Java POO"
} else {
