...
async function main() {
  ...
  const browser = await puppeteer.launch({
    ...
    timeout: 120000, // le defaut (30s) est parfois trop court quand la machine est chargee
  });

  try {
    const page = await browser.newPage();
    await page.goto(urlHtml, { waitUntil: "load", timeout: 120000 });

    await page.pdf({
      path: cheminPdf,
      format: "A4",
      printBackground: true,
      timeout: 300000, // page.pdf() a SON PROPRE defaut de 30s, distinct de protocolTimeout —
                        // trop court pour un document volumineux avec de nombreuses images
      displayHeaderFooter: true,
      headerTemplate: `<div></div>`, // vide : aucune date/heure/titre/URL automatique
      footerTemplate: `
