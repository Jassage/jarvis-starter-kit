# Build du manuel Reseaux d'entreprise & Videosurveillance IP : assemble les chapitres Markdown et genere HTML + PDF + DOCX.
...
$htmlOut = Join-Path $exportDir "manuel-reseau-entreprise.html"
$docxOut = Join-Path $exportDir "manuel-reseau-entreprise.docx"
$pdfOut  = Join-Path $exportDir "manuel-reseau-entreprise.pdf"
...
pandoc @cheminsAbsolus `
  ...
  --metadata pagetitle="Manuel complet Reseaux d'entreprise et Videosurveillance IP" `
...
pandoc @cheminsDocx `
  ...
  --toc `
  --toc-depth=2 `
  --metadata title="Manuel complet Reseaux d'entreprise et Videosurveillance IP" `
  --metadata author="Jaslin Occius" `
  --output="$docxOut"
...
if (Test-Path $nodeScript) {
  node "$nodeScript" "$htmlOut" "$pdfOut" "Jaslin Occius - Manuel Reseaux d'entreprise & Videosurveillance IP"
} else {
