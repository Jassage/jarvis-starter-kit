# Build du manuel Reseaux d'entreprise & Videosurveillance IP V3 : assemble les chapitres Markdown et genere HTML + PDF + DOCX.
...
$htmlOut = Join-Path $exportDir "manuel-reseau-entreprise-v3.html"
$docxOut = Join-Path $exportDir "manuel-reseau-entreprise-v3.docx"
$pdfOut  = Join-Path $exportDir "manuel-reseau-entreprise-v3.pdf"
...
# (indispensable ici : le scripting MikroTik RouterOS utilise "$variable" a la pelle)
# "--syntax-highlighting=breezedark" : theme de coloration pense pour un fond de code sombre
# (le theme par defaut de pandoc est trop peu contraste sur le fond sombre de style.css)
pandoc @cheminsAbsolus `
  ...
  --metadata pagetitle="Reseaux d'entreprise et Videosurveillance IP - Guide de terrain (V3)" `
...
pandoc @cheminsDocx `
  ...
  --toc `
  --toc-depth=2 `
  --metadata title="Reseaux d'entreprise et Videosurveillance IP - Guide de terrain (V3)" `
  --metadata author="Jaslin Occius" `
  --output="$docxOut"
...
if (Test-Path $nodeScript) {
  node "$nodeScript" "$htmlOut" "$pdfOut" "Jaslin Occius - Reseaux d'entreprise & Videosurveillance IP (V3)"
} else {
