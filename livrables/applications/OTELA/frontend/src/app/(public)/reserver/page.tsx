...
import { Search, Users, MapPin, CheckCircle2, ArrowLeft, Building2, Wallet, Mail, Clock3, ChevronDown } from 'lucide-react';
...
export default function ReserverPage() {
  ...
  const choisirEtablissement = (etablissementId: string) => {
    setForm((f) => ({ ...f, etablissementId }));
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16">
        <div className="card p-8 text-center">
          ...
          <button onClick={() => { setEtape('recherche'); setResultats(null); setConfirmation(null); setSelection(null); setClient({ nom: '', telephone: '', email: '' }); }} className="btn btn-primary w-full mt-6">
            Nouvelle recherche
          </button>
        </div>
      </div>
              ...
              <div className="max-w-6xl mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {etablissements.map((e) => (
                    <button key={e.id} onClick={() => choisirEtablissement(e.id)} className="card card-hover p-6 text-left">
