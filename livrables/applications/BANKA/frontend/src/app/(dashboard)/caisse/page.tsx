...
export default function CaissePage() {
  ...
  const ouvrir = async () => {
    ...
    try {
      ...
      toast.error(
        "Erreur d'ouverture",
        e.response?.data?.error || "Impossible d'ouvrir la caisse.",
      );
    } finally {
      setAction(false);
    }
  ...
  const fermer = async () => {
    ...
    try {
      ...
      toast.success(
        "Caisse fermée",
        "La session a été clôturée. Le rapport est disponible.",
      );
      setShowFermer(false);
      setSoldeFermeture("");
      await loadSession();
      ...
      toast.error(
        "Erreur de fermeture",
        e.response?.data?.error || "Impossible de fermer la caisse.",
      );
    } finally {
      setAction(false);
    }
  ...
  const handleRapport = async () => {
    ...
    try {
      generateRapportCaisse(
        session,
        utilisateur?.agence?.nom || "Siège Social",
      );
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-sm" style={{ color: "#8b94b0" }}>
          Chargement...
        </span>
      </div>
    );
  ...
  if (!utilisateur?.agenceId) {
    return (
      <div
        className="flex items-start gap-3 p-5 rounded-2xl animate-slide-up"
      ...
      >
        ...
        >
          <path
            d="M10.29 3.86l-8.18 14.14a2 2 0 001.71 3h16.36a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0zM12 9v4M12 17h.01"
            stroke="currentColor"
            strokeWidth="1.8"
            strokeLinecap="round"
        ...
        <p className="text-sm font-medium" style={{ color: "#92400e" }}>
          Vous n'êtes pas associé à une agence. Contactez un administrateur.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-5 animate-slide-up">
        ...
        <div className="card p-0 overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-4">
              ...
              >
                Caisse non ouverte pour aujourd'hui
              </span>
            </div>
          </div>
          ...
          <div className="card overflow-hidden">
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p
                    ...
                    style={{ color: "rgba(255,255,255,0.8)" }}
                  >
                    Ouverte par {session.ouvertPar?.prenom}{" "}
                    {session.ouvertPar?.nom}
                  </p>
                  <p
                ...
                <div className="text-right">
                  ...
                  <p
                    ...
                    style={{ color: "white" }}
                  >
                    {formatMontant(session.soldeOuverture, "HTG")}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              {[
                {
                  label: "Dépôts",
                  value: `+${formatMontant(totalDepots, "HTG")}`,
                  color: "#047857",
                },
                {
                  label: "Retraits",
                  value: `-${formatMontant(totalRetraits, "HTG")}`,
                  color: "#b91c1c",
                },
                {
                  label: "Solde théorique",
                  value: formatMontant(soldeTheo, "HTG"),
                  color: "#1e40af",
                },
            ...
            >
              ...
              >
                {generatingPdf ? (
                  ...
                  <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4">
                    <path
                      d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8zM14 2v6h6M12 18v-6M9 15l3 3 3-3"
                      stroke="currentColor"
                      strokeWidth="1.8"
                      strokeLinecap="round"
                      strokeLinejoin="round"
              ...
              <div>
                ...
                <div className="divide-y" style={{ borderColor: "#f0f2f9" }}>
                  {session.transactions.map((tx: any) => {
                    const isDebit = ["RETRAIT", "VIREMENT_DEBIT"].includes(
                      tx.type,
                    );
                    return (
                      <div
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            ...
                            style={{ color: "#0b1733" }}
                          >
                            {TYPE_TRANSACTION_LABELS[tx.type]}
                          </p>
                          <p
                        ...
                        <p
                          ...
                          style={{ color: isDebit ? "#b91c1c" : "#047857" }}
                        >
                          {isDebit ? "-" : "+"}
            ...
            {session.transactions?.length === 0 && (
              <div className="px-5 py-8 text-center">
                <p className="text-sm" style={{ color: "#8b94b0" }}>
                  Aucune opération enregistrée dans cette session.
                </p>
              </div>
            )}
          </div>
      ...
      {showFermer && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowFermer(false);
          }}
        >
          <div className="drawer-panel" style={{ maxWidth: "480px" }}>
            ...
            >
              ...
              <div className="flex-1">
                ...
                >
                  Clôture de session journalière
                </p>
              </div>
              <button
                onClick={() => setShowFermer(false)}
            ...
            <div className="px-5 py-5 flex-1 space-y-4">
                ...
                {[
                  {
                    ...
                    value: formatMontant(session?.soldeOuverture, "HTG"),
                    color: "#0b1733",
                  },
                  {
                    label: "Solde théorique",
              ...
              <div>
                <label className="label">
                  Solde de fermeture constaté (HTG)
                </label>
                <p className="text-xs mb-2" style={{ color: "#8b94b0" }}>
                  Comptez physiquement le contenu de la caisse.
                </p>
                <div className="relative">
                  <span
                    className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm font-semibold"
                ...
                {soldeFermeture && (
                  ...
                  >
                    Écart :{" "}
                    {parseFloat(soldeFermeture) - soldeTheo >= 0 ? "+" : ""}
                    {formatMontant(
                      parseFloat(soldeFermeture) - soldeTheo,
                      "HTG",
                    )}
                  </div>
                )}
