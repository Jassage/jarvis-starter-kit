...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(res, { users }, "Utilisateurs", 200, {
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(
      res,
      { user: updated },
      `Compte ${updated.isActive ? "activé" : "désactivé"}`,
    );
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      status &&
      !allowedListingStatuses.includes(
        status as (typeof allowedListingStatuses)[number],
      )
    ) {
      throw new AppError("Statut de listing invalide", 400);
    }
    ...
    sendSuccess(res, { listings }, "Annonces", 200, {
      pagination: paginate(page, limit, total),
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(res, { reports }, "Signalements", 200, {
      pagination: paginate(page, limit, total),
    });
  }),
);
  ...
  asyncHandler(async (req, res) => {
    const report = await prisma.report.update({
      ...
      data: {
        ...
        resolvedAt: ["RESOLVED", "DISMISSED"].includes(req.body.status)
          ? new Date()
          : undefined,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      !allowedPaymentStatuses.includes(
        query.status as (typeof allowedPaymentStatuses)[number],
      )
    ) {
    ...
    sendSuccess(res, { payments }, "Paiements", 200, {
      pagination: paginate(page, limit, total),
    });
  }),
);
