...
async function handlePspCallback(
  req: Request,
  res: Response,
  secretHeader: string,
  secret?: string,
  ...
  asyncHandler(async (_req, res) => {
    const plans = [
      ...
      },
      {
        ...
        features: [
          "Annonces illimitées",
          "180 jours par annonce",
          "3 annonces sponsorisées/mois",
          "Badge vérifié",
...
router.post(
  "/submit-proof",
  uploadAvatar.single("screenshot"),
  asyncHandler(async (req, res) => {
    const {
      planId,
      method,
    ...
    sendSuccess(
      res,
      { payment },
      "Preuve reçue. Votre plan sera activé après vérification (sous 24h).",
      201,
    );
  }),
);
