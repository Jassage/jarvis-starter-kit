...
export default function DashboardPage() {
  ...
  const [action, setAction] = useState<"vente" | "stock" | "achat" | null>(
    null,
  );
  ...
  const valeurMax = Math.max(
    1,
    ...(stats?.repartitionParEmplacement.map((r) => r.valeur) ?? [1]),
  );
  const venteMax = Math.max(
    1,
    ...(stats?.ventes7Jours.map((v) => v.montant) ?? [1]),
  );
  const produitMax = Math.max(
    1,
    ...(stats?.topProduits.map((p) => p.montantVendu) ?? [1]),
  );
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      >
        ...
        />
        <svg
          className="absolute -top-16 -right-10 opacity-[0.12] pointer-events-none"
          width="280"
          height="280"
          viewBox="0 0 280 280"
          fill="none"
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <div
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-3 text-[11px] font-bold tracking-widest"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 sm:gap-5">
        ...
        <StatCard
          ...
          sub={
            stats && stats.clientsRisqueCount > 0
              ? `${stats.clientsRisqueCount} client(s) à solde positif`
              : "Aucun encours"
          }
        />
        <StatCard
          ...
          sub={
            stats && stats.produitsSousAlerte > 0
              ? "À traiter sur la page Stock"
      ...
      {stats && stats.commandesEnAttente > 0 && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              {stats.commandesEnAttente} commande
              {stats.commandesEnAttente > 1 ? "s" : ""} en attente de réception
              {stats.commandesEnRetard > 0 &&
                ` · ${stats.commandesEnRetard} en retard sur la livraison prévue`}
            </p>
          </div>
          <Link
            href="/achats"
        ...
        <div className="xl:col-span-2 card p-5 sm:p-6">
          ...
          <div className="flex items-end justify-between gap-2 sm:gap-3 h-40">
            {stats?.ventes7Jours.map((v) => {
              ...
              const jour = new Date(v.date + "T12:00:00").toLocaleDateString(
                "fr-FR",
                { weekday: "short" },
              );
              return (
                ...
                >
                  ...
                  >
                    {v.montant > 0
                      ? formatMontantCompact(v.montant).replace(" HTG", "")
                      : ""}
                  </span>
                  <div
                ...
                <div key={r.id}>
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${Math.max(4, (r.valeur / valeurMax) * 100)}%`,
                        background: "var(--gradient-brand)",
                      }}
                    />
                  </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 sm:gap-6">
        <div className="card p-5 sm:p-6">
          <h3
            className="text-sm font-bold tracking-wide mb-5 flex items-center gap-2"
            style={{ color: "var(--color-ink)" }}
          >
            <Trophy
              className="w-4 h-4"
            ...
            {stats?.topProduits.map((p, i) => (
              <div key={p.produitId} className="flex items-center gap-3">
                <span
                  ...
                  style={{
                    background: RANG_STYLE[i]?.bg ?? "var(--color-line-2)",
                    color: RANG_STYLE[i]?.fg ?? "var(--color-ink-3)",
                  }}
                >
                  {i + 1}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      ...
                      style={{ color: "var(--color-ink)" }}
                    >
                      {formatMontantCompact(p.montantVendu)}
                    </span>
                  </div>
                  <div
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${Math.max(4, (p.montantVendu / produitMax) * 100)}%`,
                  ...
                  <p
                    ...
                    style={{ color: "var(--color-ink-3)" }}
                  >
                    {p.quantiteVendue} {p.unite} vendu(e)s
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="card p-5 sm:p-6">
          ...
          <div className="space-y-1">
            ...
            {(!stats || stats.clientsRisque.length === 0) && (
              <p className="text-sm" style={{ color: "var(--color-ink-3)" }}>
                Aucun client à solde dû actuellement.
              </p>
            )}
          </div>
          {stats && stats.clientsRisqueCount > 0 && (
      ...
      >
        <NouvelleVenteModal
          onDone={() => handleDone("Vente enregistrée avec succès.")}
        />
      </Modal>
      <Modal
        open={action === "stock"}
        ...
        maxWidth={540}
      >
        <QuickAjustementModal
          onDone={() => handleDone("Stock ajusté avec succès.")}
        />
      </Modal>
      <Modal
        open={action === "achat"}
      ...
      >
        <NouvelleCommandeModal
          onDone={() => handleDone("Commande créée avec succès.")}
        />
      </Modal>
    </div>
  );
