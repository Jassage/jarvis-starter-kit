...
export default function AchatsTab() {
  const { achats, isLoading, error, fetchAchats } = useRapportStore();
  ...
  useEffect(() => {
    void fetchAchats({ from, to }).catch(() => undefined);
  }, [fetchAchats, from, to]);
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
            <StatCard
              ...
              value={`${formatMontant(achats.montantCommande)} HTG`}
            />
            <StatCard
              icon={PackageCheck}
              theme="blue"
              label="MONTANT REÇU"
              value={`${formatMontant(achats.montantRecu)} HTG`}
            />
            <StatCard
              icon={Percent}
              theme="violet"
              label="TAUX DE RÉCEPTION"
              value={`${achats.tauxReception}%`}
            />
            <StatCard
              icon={AlertTriangle}
              theme="rose"
              label="COMMANDES EN RETARD"
              value={String(achats.commandesEnRetard.length)}
            />
          </div>
          ...
          <div className="card overflow-hidden">
                  ...
                  <tr key={f.fournisseurId}>
                    ...
                    <td className="text-right whitespace-nowrap">
                      <p
                        ...
                        style={{ color: "var(--color-ink)" }}
                      >
                        {formatMontantCompact(f.montantCommande)}
                      </p>
                      <p
                        className="text-xs"
                        style={{ color: "var(--color-ink-3)" }}
                      >
                        reçu {formatMontantCompact(f.montantRecu)}
                      </p>
                    </td>
                  </tr>
                ))}
            ...
            {achats.topFournisseurs.length === 0 && (
          ...
          <div className="card overflow-hidden">
                ...
                {achats.commandesEnRetard.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="text-right whitespace-nowrap">
                      ...
                      >
                        prévue{" "}
                        {new Date(c.dateLivraisonPrevue).toLocaleDateString(
                          "fr-FR",
                        )}
                      </p>
                    </td>
            ...
            {achats.commandesEnRetard.length === 0 && (
              <EmptyState title="Aucune commande en retard" />
            )}
          </div>
        </>
      )}
