...
function defaultPeriode(from?: Date, to?: Date): { from: Date; to: Date } {
  ...
  const fromDate = from ?? new Date(toDate.getTime() - 29 * 24 * 60 * 60 * 1000);
  fromDate.setHours(0, 0, 0, 0);
  toDate.setHours(23, 59, 59, 999);
  return { from: fromDate, to: toDate };
}

export async function getRapportVentes(params: { from?: Date; to?: Date; emplacementId?: string }) {
  const { from, to } = defaultPeriode(params.from, params.to);
  const whereVente: any = { statut: 'VALIDEE', dateVente: { gte: from, lte: to } };
  ...
  const [ventes, lignes, parModePaiement] = await Promise.all([
    ...
    prisma.ligneVente.findMany({
      ...
      select: {
        ...
        montantLigne: true,
        prixUnitaire: true,
        produit: { select: { id: true, nom: true, reference: true, unite: true, prixAchatMoyen: true } },
      },
    }),
    prisma.vente.groupBy({ by: ['modePaiement'], where: whereVente, _sum: { montantTotal: true }, _count: true }),
  ]);
  ...
  const evolution = Array.from(buckets.entries()).map(([date, v]) => ({ date, montant: v.montant, count: v.count }));

  // Top produits + marge estimée sur le prix d'achat moyen courant (pas d'historique de coût stocké)
  const parProduit = new Map<
    string,
    { nom: string; reference: string; unite: string; quantiteVendue: number; montantVendu: number; margeEstimee: number }
  >();
  for (const l of lignes) {
    ...
    entry.margeEstimee += l.quantite * (Number(l.prixUnitaire) - Number(l.produit.prixAchatMoyen));
  ...
  const topProduits = Array.from(parProduit.entries())
    ...
    .slice(0, 10);
  const margeTotaleEstimee = Array.from(parProduit.values()).reduce((sum, v) => sum + v.margeEstimee, 0);

  // Top clients
  const parClient = new Map<string, { nom: string; type: string; montantAchete: number; nombreAchats: number }>();
  for (const v of ventes) {
    if (!v.client) continue;
    const entry = parClient.get(v.client.id) ?? { nom: v.client.nom, type: v.client.type, montantAchete: 0, nombreAchats: 0 };
...
export async function getRapportStock() {
  const [stocks, mouvements90j] = await Promise.all([
    ...
    prisma.mouvementStock.findMany({
      where: { type: { in: ['VENTE', 'SORTIE'] }, createdAt: { gte: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000) } },
      select: { produitId: true, quantite: true },
    }),
  ]);

  const stocksActifs = stocks.filter((s) => s.produit.actif);
  const valeurStockTotal = stocksActifs.reduce((sum, s) => sum + s.quantite * Number(s.produit.prixAchatMoyen), 0);

  const parEmplacement = new Map<string, { nom: string; type: string; quantite: number; valeur: number }>();
  for (const s of stocksActifs) {
    const entry = parEmplacement.get(s.emplacementId) ?? { nom: s.emplacement.nom, type: s.emplacement.type, quantite: 0, valeur: 0 };
    entry.quantite += s.quantite;
    entry.valeur += s.quantite * Number(s.produit.prixAchatMoyen);
    parEmplacement.set(s.emplacementId, entry);
  }
  const valorisationParEmplacement = Array.from(parEmplacement.entries()).map(([id, v]) => ({ id, ...v }));
  ...
  const valorisationParCategorie = Array.from(parCategorie.entries()).map(([categorie, v]) => ({ categorie, ...v }));
  ...
  for (const m of mouvements90j) {
    sortiesParProduit.set(m.produitId, (sortiesParProduit.get(m.produitId) ?? 0) + Math.abs(m.quantite));
  }

  const produitVu = new Map<string, { nom: string; reference: string; quantiteStock: number }>();
  for (const s of stocksActifs) {
    const entry = produitVu.get(s.produitId) ?? { nom: s.produit.nom, reference: s.produit.reference, quantiteStock: 0 };
...
export async function getRapportAchats(params: { from?: Date; to?: Date }) {
  ...
  let montantRecu = 0;
  const parFournisseur = new Map<string, { nom: string; montantCommande: number; montantRecu: number; nombreCommandes: number }>();

  for (const c of commandes) {
    const totalCommande = c.lignes.reduce((sum, l) => sum + l.quantiteCommandee * Number(l.prixUnitaireAchat), 0);
    const totalRecu = c.lignes.reduce((sum, l) => sum + l.quantiteRecue * Number(l.prixUnitaireAchat), 0);
  ...
  const commandesEnRetard = commandes
    .filter((c) => ['ENVOYEE', 'RECUE_PARTIELLE'].includes(c.statut) && c.dateLivraisonPrevue && c.dateLivraisonPrevue < maintenant)
    .map((c) => ({ id: c.id, numero: c.numero, fournisseur: c.fournisseur.nom, dateLivraisonPrevue: c.dateLivraisonPrevue, statut: c.statut }));

  const ventilationStatut = commandes.reduce<Record<string, number>>((acc, c) => {
  ...
  return {
    ...
    montantCommande,
    montantRecu,
    tauxReception: montantCommande > 0 ? Math.round((montantRecu / montantCommande) * 1000) / 10 : 0,
...
export async function getRapportClients() {
  const [clients, ventes] = await Promise.all([
    ...
    prisma.vente.findMany({ where: { statut: 'VALIDEE', clientId: { not: null } }, select: { clientId: true, montantTotal: true } }),
  ]);

  const encoursCreditTotal = clients.reduce((sum, c) => sum + Number(c.soldeDu), 0);
  ...
  const ventilationParType = Array.from(parType.entries()).map(([type, v]) => ({ type, ...v }));

  const topClientsSoldeDu = clients
    .filter((c) => Number(c.soldeDu) > 0)
    .sort((a, b) => Number(b.soldeDu) - Number(a.soldeDu))
    .slice(0, 10)
    .map((c) => ({ id: c.id, nom: c.nom, type: c.type, soldeDu: Number(c.soldeDu) }));

  const achatsParClient = new Map<string, number>();
  for (const v of ventes) {
    if (!v.clientId) continue;
    achatsParClient.set(v.clientId, (achatsParClient.get(v.clientId) ?? 0) + Number(v.montantTotal));
  }
  const topClientsAcheteurs = clients
    .map((c) => ({ id: c.id, nom: c.nom, type: c.type, montantAchete: achatsParClient.get(c.id) ?? 0 }))
