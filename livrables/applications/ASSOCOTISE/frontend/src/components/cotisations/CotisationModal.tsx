...
import { saisirCotisation } from '../../services/cotisations.service';
import { useAuth } from '../../contexts/AuthContext';
import type { Membre, MoyenPaiement } from '../../types';
...
export function CotisationModal({
  ...
  /** Si fourni, ce champ est prérempli et le titre indique qu'il s'agit d'une correction */
  correction?: boolean;
}) {
  const { profil } = useAuth();
  const [memberId, setMemberId] = useState(membreParDefaut ?? '');
  const [mois, setMois] = useState(moisParDefaut);
  const [montant, setMontant] = useState(MONTANT_MINIMUM);
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [moyenPaiement, setMoyenPaiement] = useState<MoyenPaiement>('cash');
